...
當 `preserveKeys` 屬性設置為 `true` 時，當集合從路由或控制器返回時，集合鍵值將被保留：
...
> [!NOTE]
> 如果您尚未閱讀 [概念概覽](#concept-overview) 章節，強烈建議您在繼續閱讀本文件前先行閱讀。
...
> [!NOTE]
> 如果您只想在關聯已被載入時才包含它，請查閱 [條件式關聯](#conditional-relationships) 的文件。
...
如果您想停用最外層資源的包裝，您應該在基礎 `Illuminate\Http\Resources\Json\JsonResource` 類別上調用 `withoutWrapping` 方法。通常，您應該在 `AppServiceProvider` 或另一個在應用程式的每個請求上載入的 [服務提供者](/docs/{{version}}/providers) 中調用此方法：
...
當透過資源回應返回分頁集合時，即使已調用 `withoutWrapping` 方法，Laravel 仍會將您的資源資料包裝在 `data` 鍵中。這是因為分頁回應總是包含 `meta` 和 `links` 鍵，其中包含關於分頁器狀態的資訊：
...
分頁回應始終包含帶有分頁器狀態資訊的 `meta` 和 `links` 鍵：
...
如果您想自訂分頁回應中 `links` 或 `meta` 鍵所包含的資訊，您可以在資源上定義 `paginationInformation` 方法。此方法將接收 `$paginated` 資料和 `$default` 資訊陣列，後者是一個包含 `links` 和 `meta` 鍵的陣列：
...
如果您的關聯正在使用 [自訂中介表格 Model](/docs/{{version}}/eloquent-relationships#defining-custom-intermediate-table-models)，您可以將中介表格 Model 的實例作為第一個引數傳遞給 `whenPivotLoaded` 方法：
...
從您的資源回傳額外中繼資料時，您無需擔心意外覆寫 Laravel 在回傳分頁回應時自動新增的 `links` 或 `meta` 鍵。您定義的任何額外 `links` 都會與分頁器提供的連結合併。
