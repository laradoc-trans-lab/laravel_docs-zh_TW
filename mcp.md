# Laravel MCP

- [簡介](#introduction)
- [安裝](#installation)
    - [發布路由](#publishing-routes)
- [建立伺服器](#creating-servers)
    - [伺服器註冊](#server-registration)
    - [Web 伺服器](#web-servers)
    - [本地伺服器](#local-servers)
- [工具 (Tools)](#tools)
    - [建立工具](#creating-tools)
    - [工具輸入結構 (Input Schemas)](#tool-input-schemas)
    - [工具輸出結構 (Output Schemas)](#tool-output-schemas)
    - [驗證工具參數](#validating-tool-arguments)
    - [工具依賴注入](#tool-dependency-injection)
    - [工具註解 (Annotations)](#tool-annotations)
    - [條件式工具註冊](#conditional-tool-registration)
    - [工具回應](#tool-responses)
- [提示 (Prompts)](#prompts)
    - [建立提示](#creating-prompts)
    - [提示參數](#prompt-arguments)
    - [驗證提示參數](#validating-prompt-arguments)
    - [提示依賴注入](#prompt-dependency-injection)
    - [條件式提示註冊](#conditional-prompt-registration)
    - [提示回應](#prompt-responses)
- [資源 (Resources)](#resources)
    - [建立資源](#creating-resources)
    - [資源模板](#resource-templates)
    - [資源 URI 與 MIME 類型](#resource-uri-and-mime-type)
    - [資源請求](#resource-request)
    - [資源依賴注入](#resource-dependency-injection)
    - [資源註解 (Annotations)](#resource-annotations)
    - [條件式資源註冊](#conditional-resource-registration)
    - [資源回應](#resource-responses)
- [中繼資料 (Metadata)](#metadata)
- [身份驗證](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [授權](#authorization)
- [測試伺服器](#testing-servers)
    - [MCP Inspector](#mcp-inspector)
    - [單元測試](#unit-tests)

<a name="introduction"></a>
## 簡介

[Laravel MCP](https://github.com/laravel/mcp) 為 AI 客戶端提供了一種簡單且優雅的方式，透過 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) 與您的 Laravel 應用程式進行互動。它提供了一個具表現力且流暢的介面來定義伺服器、工具、資源和提示 (Prompts)，進而實現應用程式中由 AI 驅動的互動。


<a name="installation"></a>
## 安裝

若要開始使用，請使用 Composer 套件管理器將 Laravel MCP 安裝到您的專案中：

```shell
composer require laravel/mcp
```


<a name="publishing-routes"></a>
### 發布路由

安裝 Laravel MCP 後，執行 `vendor:publish` Artisan 指令來發布 `routes/ai.php` 檔案，您將在該檔案中定義您的 MCP 伺服器：

```shell
php artisan vendor:publish --tag=ai-routes
```

此指令會在您應用程式的 `routes` 目錄中建立 `routes/ai.php` 檔案，您將使用它來註冊您的 MCP 伺服器。


<a name="creating-servers"></a>
## 建立伺服器

您可以使用 `make:mcp-server` Artisan 指令建立一個 MCP 伺服器。伺服器作為中央通訊點，向 AI 客戶端公開工具、資源和提示等 MCP 功能：

```shell
php artisan make:mcp-server WeatherServer
```

此指令將在 `app/Mcp/Servers` 目錄中建立一個新的伺服器類別。生成的伺服器類別繼承了 Laravel MCP 的基礎 `Laravel\Mcp\Server` 類別，並提供用於配置伺服器以及註冊工具、資源和提示的屬性與特性：

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server\Attributes\Instructions;
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Version;
use Laravel\Mcp\Server;

#[Name('Weather Server')]
#[Version('1.0.0')]
#[Instructions('This server provides weather information and forecasts.')]
class WeatherServer extends Server
{
    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // GetCurrentWeatherTool::class,
    ];

    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // WeatherGuidelinesResource::class,
    ];

    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // DescribeWeatherPrompt::class,
    ];
}
```


<a name="server-registration"></a>
### 伺服器註冊

建立伺服器後，您必須在 `routes/ai.php` 檔案中註冊它以供存取。Laravel MCP 提供了兩種註冊伺服器的方法：`web` 用於可透過 HTTP 存取的伺服器，以及 `local` 用於命令列伺服器。


<a name="web-servers"></a>
### Web 伺服器

Web 伺服器是最常見的伺服器類型，可透過 HTTP POST 請求進行存取，非常適合遠端 AI 客戶端或基於 Web 的整合。使用 `web` 方法註冊 Web 伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

就像一般路由一樣，您可以套用中介層 (middleware) 來保護您的 Web 伺服器：

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```


<a name="local-servers"></a>
### 本地伺服器

本地伺服器以 Artisan 指令形式運行，非常適合建立本地 AI 助手整合，例如 [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost)。使用 `local` 方法註冊本地伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

註冊後，您通常不需要手動自行執行 `mcp:start` Artisan 指令。相反地，請配置您的 MCP 客戶端 (AI 代理程式) 來啟動伺服器，或使用 [MCP Inspector](#mcp-inspector)。

<a name="tools"></a>
## 工具 (Tools)

工具可讓您的伺服器公開 AI 用戶端可以呼叫的功能。它們允許語言模型執行動作、執行程式碼或與外部系統進行互動：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // Get weather...

        return Response::text('The weather is...');
    }

    /**
     * Get the tool's input schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```


<a name="creating-tools"></a>
### 建立工具

要建立工具，請執行 `make:mcp-tool` Artisan 指令：

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

建立工具後，請將其註冊在伺服器的 `$tools` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```


<a name="tool-name-title-description"></a>
#### 工具名稱、標題與描述

預設情況下，工具的名稱和標題是從類別名稱衍生而來的。例如，`CurrentWeatherTool` 的名稱將會是 `current-weather`，標題則是 `Current Weather Tool`。您可以使用 `Name` 和 `Title` 屬性 (Attributes) 來自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('get-optimistic-weather')]
#[Title('Get Optimistic Weather Forecast')]
class CurrentWeatherTool extends Tool
{
    // ...
}
```

工具描述不會自動產生。您應該始終使用 `Description` 屬性來提供具有意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    //
}
```

> [!NOTE]
> 描述是工具中繼資料的重要部分，因為它能幫助 AI 模型理解何時以及如何有效地使用該工具。


<a name="tool-input-schemas"></a>
### 工具輸入結構 (Input Schemas)

工具可以定義輸入結構，以指定它們接受來自 AI 用戶端的哪些參數。使用 Laravel 的 `Illuminate\Contracts\JsonSchema\JsonSchema` 構建器來定義工具的輸入需求：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Get the tool's input schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->string()
                ->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```


<a name="tool-output-schemas"></a>
### 工具輸出結構 (Output Schemas)

工具可以定義 [輸出結構 (Output Schemas)](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema) 以指定其回應的結構。這能與需要解析工具結果的 AI 用戶端進行更好的整合。使用 `outputSchema` 方法來定義工具的輸出結構：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Get the tool's output schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function outputSchema(JsonSchema $schema): array
    {
        return [
            'temperature' => $schema->number()
                ->description('Temperature in Celsius')
                ->required(),

            'conditions' => $schema->string()
                ->description('Weather conditions')
                ->required(),

            'humidity' => $schema->integer()
                ->description('Humidity percentage')
                ->required(),
        ];
    }
}
```


<a name="validating-tool-arguments"></a>
### 驗證工具參數

JSON Schema 定義提供了工具參數的基本結構，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在工具的 `handle` 方法中驗證傳入的工具參數：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // Fetch weather data using the validated arguments...
    }
}
```

當驗證失敗時，AI 用戶端將根據您提供的錯誤訊息採取行動。因此，提供清晰且可操作的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```


<a name="tool-dependency-injection"></a>
#### 工具依賴注入

Laravel [服務容器](/docs/{{version}}/container) 用於解析所有工具。因此，您可以在建構函式中對工具可能需要的任何依賴項進行型別提示 (Type-hint)。宣告的依賴項將自動被解析並注入到工具實例中：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Create a new tool instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構函式注入外，您還可以在工具的 `handle()` 方法中對依賴項進行型別提示。當方法被呼叫時，服務容器將自動解析並注入依賴項：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```

<a name="tool-annotations"></a>
### 工具註解 (Annotations)

您可以透過 [註解 (annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 來增強工具，為 AI 客戶端提供額外の中繼資料。這些註解有助於 AI 模型理解工具的行為與功能。註解是透過屬性 (attributes) 加入到工具中的：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

可用的註解包括：

| 註解 (Annotation) | 類型 | 描述 |
| ------------------ | ------- | -------------------------------------------------------------------------------------------- |
| `#[IsReadOnly]`    | boolean | 表示該工具不會修改其環境。 |
| `#[IsDestructive]` | boolean | 表示該工具可能會執行破壞性更新（僅在非唯讀時有意義）。 |
| `#[IsIdempotent]`  | boolean | 表示使用相同參數重複呼叫不會產生額外影響（當非唯讀時）。 |
| `#[IsOpenWorld]`   | boolean | 表示該工具可能會與外部實體互動。 |

註解值可以使用布林參數明確設定：

```php
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tools\Annotations\IsDestructive;
use Laravel\Mcp\Server\Tools\Annotations\IsOpenWorld;
use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tool;

#[IsReadOnly(true)]
#[IsDestructive(false)]
#[IsOpenWorld(false)]
#[IsIdempotent(true)]
class CurrentWeatherTool extends Tool
{
    //
}
```


<a name="conditional-tool-registration"></a>
### 條件式工具註冊

您可以透過在工具類別中實作 `shouldRegister` 方法，在執行時期條件式地註冊工具。此方法允許您根據應用程式狀態、設定或請求參數來決定工具是否可用：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Determine if the tool should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當工具的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用工具清單中，且無法被 AI 客戶端調用。


<a name="tool-responses"></a>
### 工具回應

工具必須回傳 `Laravel\Mcp\Response` 的執行個體。Response 類別提供了幾個方便的方法來建立不同類型的回應：

對於簡單的文字回應，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

若要表示工具執行期間發生錯誤，請使用 `error` 方法：

```php
return Response::error('Unable to fetch weather data. Please try again.');
```


<a name="multiple-content-responses"></a>
#### 多重內容回應

工具可以藉由回傳 `Response` 執行個體的陣列來回傳多個內容片段：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text('**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F')
    ];
}
```


<a name="structured-responses"></a>
#### 結構化回應

工具可以使用 `structured` 方法回傳 [結構化內容](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content)。這為 AI 客戶端提供了可解析的資料，同時保持與 JSON 編碼文字表示方式的向後相容性：

```php
return Response::structured([
    'temperature' => 22.5,
    'conditions' => 'Partly cloudy',
    'humidity' => 65,
]);
```

如果您需要在結構化內容旁提供自訂文字，請在回應工廠 (response factory) 使用 `withStructuredContent` 方法：

```php
return Response::make(
    Response::text('Weather is 22.5°C and sunny')
)->withStructuredContent([
    'temperature' => 22.5,
    'conditions' => 'Sunny',
]);
```


<a name="streaming-responses"></a>
#### 串流回應

對於長時間運行的操作或即時資料串流，工具可以從其 `handle` 方法回傳一個 [產生器 (generator)](https://www.php.net/manual/en/language.generators.overview.php)。這使得在最終回應之前可以向客戶端傳送中間更新：

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

當使用 Web 型伺服器時，串流回應會自動開啟 SSE (Server-Sent Events) 串流，將每個產出的訊息作為事件傳送到客戶端。

<a name="prompts"></a>
## 提示 (Prompts)

[提示 (Prompts)](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 讓您的伺服器能夠共享可重複使用的提示模板，AI 客戶端可以使用這些模板與語言模型進行互動。它們提供了一種標準化方式來結構化常見的查詢與互動。


<a name="creating-prompts"></a>
### 建立提示

若要建立提示，請執行 `make:mcp-prompt` Artisan 指令：

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

建立提示後，請將其註冊到伺服器的 `$prompts` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```


<a name="prompt-name-title-and-description"></a>
#### 提示名稱、標題與描述

預設情況下，提示的名稱和標題源自於類別名稱。例如，`DescribeWeatherPrompt` 將具有名稱 `describe-weather` 和標題 `Describe Weather Prompt`。您可以使用 `Name` 和 `Title` 屬性自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-assistant')]
#[Title('Weather Assistant Prompt')]
class DescribeWeatherPrompt extends Prompt
{
    // ...
}
```

提示描述不會自動生成。您應該始終使用 `Description` 屬性提供有意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Generates a natural-language explanation of the weather for a given location.')]
class DescribeWeatherPrompt extends Prompt
{
    //
}
```

> [!NOTE]
> 描述是提示中繼資料的關鍵部分，因為它能幫助 AI 模型了解何時以及如何最佳化地使用該提示。


<a name="prompt-arguments"></a>
### 提示參數

提示可以定義參數，讓 AI 客戶端能使用特定值來自定義提示模板。使用 `arguments` 方法來定義您的提示所接受的參數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Get the prompt's arguments.
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```


<a name="validating-prompt-arguments"></a>
### 驗證提示參數

提示參數會根據其定義自動驗證，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的[驗證功能](/docs/{{version}}/validation)無縫整合。您可以在提示的 `handle` 方法中驗證傳入的提示參數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // Generate the prompt response using the given tone...
    }
}
```

當驗證失敗時， AI 客戶端將根據您提供的錯誤訊息進行操作。因此，提供清晰且可操作的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```


<a name="prompt-dependency-injection"></a>
### 提示依賴注入

Laravel [服務容器](/docs/{{version}}/container) 用於解析所有提示。因此，您可以在提示的建構子中對所需的任何依賴項進行型別提示。宣告的依賴項將自動被解析並注入到提示實例中：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Create a new prompt instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

除了建構子注入外，您也可以在提示的 `handle` 方法中對依賴項進行型別提示。當方法被呼叫時，服務容器將自動解析並注入依賴項：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```


<a name="conditional-prompt-registration"></a>
### 條件式提示註冊

您可以透過在提示類別中實作 `shouldRegister` 方法，在執行時期條件式地註冊提示。此方法讓您能根據應用程式狀態、配置或請求參數來決定提示是否可用：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * Determine if the prompt should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當提示的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用提示清單中，且無法被 AI 客戶端調用。


<a name="prompt-responses"></a>
### 提示回應

提示可以回傳單個 `Laravel\Mcp\Response` 或 `Laravel\Mcp\Response` 實例的迭代對象。這些回應封裝了將發送給 AI 客戶端的內容：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

您可以使用 `asAssistant()` 方法來指示回應訊息應被視為來自 AI 助手，而一般訊息則被視為使用者輸入。


<a name="resources"></a>
## 資源 (Resources)

[資源 (Resources)](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 讓您的伺服器能夠公開資料和內容，供 AI 客戶端在與語言模型互動時讀取並作為上下文使用。它們提供了一種分享靜態或動態資訊的方式，例如文件、配置或任何有助於提供 AI 回應資訊的資料。

<a name="creating-resources"></a>
## 建立資源

要建立資源，請執行 `make:mcp-resource` Artisan 指令：

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

建立資源後，請在伺服器的 `$resources` 屬性中註冊它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```


<a name="resource-name-title-and-description"></a>
#### 資源名稱、標題與描述

預設情況下，資源的名稱和標題是根據類別名稱衍生而來的。例如，`WeatherGuidelinesResource` 的名稱將為 `weather-guidelines`，標題為 `Weather Guidelines Resource`。您可以使用 `Name` 和 `Title` 屬性來自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-api-docs')]
#[Title('Weather API Documentation')]
class WeatherGuidelinesResource extends Resource
{
    // ...
}
```

資源描述不會自動產生。您應該始終使用 `Description` 屬性提供具備意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Comprehensive guidelines for using the Weather API.')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```

> [!NOTE]
> 描述是資源中繼資料的關鍵部分，因為它能幫助 AI 模型理解何時以及如何有效地使用該資源。


<a name="resource-templates"></a>
### 資源模板

[資源模板](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates) 讓您的伺服器能夠公開與帶有變數的 URI 模式相匹配的動態資源。您不需要為每個資源定義靜態 URI，而是可以建立一個單一資源，根據模板模式處理多個 URI。


<a name="creating-resource-templates"></a>
#### 建立資源模板

要建立資源模板，請在您的資源類別中實作 `HasUriTemplate` 介面，並定義一個回傳 `UriTemplate` 實例的 `uriTemplate` 方法：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

#[Description('Access user files by ID')]
#[MimeType('text/plain')]
class UserFileResource extends Resource implements HasUriTemplate
{
    /**
     * Get the URI template for this resource.
     */
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/files/{fileId}');
    }

    /**
     * Handle the resource request.
     */
    public function handle(Request $request): Response
    {
        $userId = $request->get('userId');
        $fileId = $request->get('fileId');

        // Fetch and return the file content...

        return Response::text($content);
    }
}
```

當資源實作了 `HasUriTemplate` 介面時，它將被註冊為資源模板而非靜態資源。AI 用戶端隨後可以使用與模板模式匹配的 URI 請求資源，系統會自動從 URI 中擷取變數，並使其在資源的 `handle` 方法中可用。


<a name="uri-template-syntax"></a>
#### URI 模板語法

URI 模板使用包圍在大括號中的佔位符來定義 URI 中的變數片段：

```php
new UriTemplate('file://users/{userId}');
new UriTemplate('file://users/{userId}/files/{fileId}');
new UriTemplate('https://api.example.com/{version}/{resource}/{id}');
```


<a name="accessing-template-variables"></a>
#### 存取模板變數

當 URI 與您的資源模板匹配時，擷取出的變數會自動合併到請求中，並且可以使用 `get` 方法進行存取：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

class UserProfileResource extends Resource implements HasUriTemplate
{
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/profile');
    }

    public function handle(Request $request): Response
    {
        // Access the extracted variable
        $userId = $request->get('userId');

        // Access the full URI if needed
        $uri = $request->uri();

        // Fetch user profile...

        return Response::text("Profile for user {$userId}");
    }
}
```

`Request` 物件提供了擷取的變數以及原始請求的 URI，為您處理資源請求提供完整的上下文。


<a name="resource-uri-and-mime-type"></a>
### 資源 URI 與 MIME 類型

每個資源都由唯一的 URI 識別，並具有關聯的 MIME 類型，這有助於 AI 用戶端理解資源的格式。

預設情況下，資源的 URI 是根據資源名稱產生的，因此 `WeatherGuidelinesResource` 的 URI 將為 `weather://resources/weather-guidelines`。預設的 MIME 類型為 `text/plain`。

您可以使用 `Uri` 和 `MimeType` 屬性來自定義這些值：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Attributes\Uri;
use Laravel\Mcp\Server\Resource;

#[Uri('weather://resources/guidelines')]
#[MimeType('application/pdf')]
class WeatherGuidelinesResource extends Resource
{
}
```

URI 和 MIME 類型有助於 AI 用戶端決定如何適當地處理和解析資源內容。


<a name="resource-request"></a>
### 資源請求

與工具和提示不同，資源無法定義輸入結構 (Schemas) 或參數。不過，您仍然可以在資源的 `handle` 方法中與請求物件互動：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```


<a name="resource-dependency-injection"></a>
### 資源依賴注入

Laravel [服務容器](/docs/{{version}}/container) 用於解析所有資源。因此，您可以在資源的建構子中對資源可能需要的任何依賴進行型別提示。宣告的依賴將自動被解析並注入到資源實例中：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Create a new resource instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構子注入外，您也可以在資源的 `handle` 方法中對依賴進行型別提示。當該方法被呼叫時，服務容器將自動解析並注入這些依賴：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="resource-annotations"></a>
### 資源註解 (Annotations)

您可以使用 [註解 (annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#resourceannotations) 來增強您的資源，以向 AI 客戶端提供額外の中繼資料。註解是透過屬性 (Attributes) 新增到資源中的：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Enums\Role;
use Laravel\Mcp\Server\Annotations\Audience;
use Laravel\Mcp\Server\Annotations\LastModified;
use Laravel\Mcp\Server\Annotations\Priority;
use Laravel\Mcp\Server\Resource;

#[Audience(Role::User)]
#[LastModified('2025-01-12T15:00:58Z')]
#[Priority(0.9)]
class UserDashboardResource extends Resource
{
    //
}
```

可用的註解包括：

| 註解 (Annotation) | 類型 | 描述 |
| ----------------- | ------------- | --------------------------------------------------------------------------- |
| `#[Audience]`     | Role 或 array | 指定預期的受眾 (`Role::User`、`Role::Assistant` 或兩者)。 |
| `#[Priority]`     | float         | 一個介於 0.0 到 1.0 之間的數值，表示資源的重要性。 |
| `#[LastModified]` | string        | 一個 ISO 8601 時間戳記，顯示資源最後更新的時間。 |


<a name="conditional-resource-registration"></a>
### 條件式資源註冊

您可以透過在資源類別中實作 `shouldRegister` 方法，在執行時期條件式地註冊資源。此方法允許您根據應用程式狀態、設定或請求參數來決定資源是否可用：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Determine if the resource should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當資源的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用資源清單中，且 AI 客戶端無法存取。


<a name="resource-responses"></a>
### 資源回應

資源必須回傳 `Laravel\Mcp\Response` 的實例。Response 類別提供了幾種方便的方法來建立不同類型的回應：

對於簡單的文字內容，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the resource request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```


<a name="resource-blob-responses"></a>
#### Blob 回應

若要回傳 blob 內容，請使用 `blob` 方法並提供 blob 內容：

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

回傳 blob 內容時，MIME 類型將由您資源設定的 MIME 類型決定：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Resource;

#[MimeType('image/png')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```


<a name="resource-error-responses"></a>
#### 錯誤回應

若要表示在資源獲取過程中發生錯誤，請使用 `error()` 方法：

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="metadata"></a>
## 中繼資料 (Metadata)

Laravel MCP 也支援 [MCP 規範](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta)中定義的 `_meta` 欄位，這是某些 MCP 客戶端或整合所需要的。中繼資料可以套用到所有 MCP 原語，包含工具、資源與提示，以及它們的回應。

您可以使用 `withMeta` 方法將中繼資料附加到個別的回應內容：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    return Response::text('The weather is sunny.')
        ->withMeta(['source' => 'weather-api', 'cached' => true]);
}
```

對於套用到整個回應封包的結果層級中繼資料，請使用 `Response::make` 包裝您的回應，並在回傳的回應工廠實例上呼叫 `withMeta`：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\ResponseFactory;

/**
 * Handle the tool request.
 */
public function handle(Request $request): ResponseFactory
{
    return Response::make(
        Response::text('The weather is sunny.')
    )->withMeta(['request_id' => '12345']);
}
```

若要將中繼資料附加到工具、資源或提示本身，請在類別上定義 `$meta` 屬性：

```php
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast.')]
class CurrentWeatherTool extends Tool
{
    protected ?array $meta = [
        'version' => '2.0',
        'author' => 'Weather Team',
    ];

    // ...
}
```

<a name="authentication"></a>
## 身份驗證

就像路由一樣，您可以使用中介層對 web MCP 伺服器進行身份驗證。為您的 MCP 伺服器加入身份驗證後，使用者在能夠使用伺服器的任何功能之前，都必須先進行身份驗證。

驗證您的 MCP 伺服器存取權限有兩種方式：透過 [Laravel Sanctum](/docs/{{version}}/sanctum) 進行簡單、基於權杖 (token) 的身份驗證，或是任何透過 `Authorization` HTTP 標頭傳遞的權杖。或者，您也可以使用 [Laravel Passport](/docs/{{version}}/passport) 透過 OAuth 進行身份驗證。

<a name="oauth"></a>
### OAuth 2.1

保護您的 Web 型 MCP 伺服器最穩固的方法是使用 [Laravel Passport](/docs/{{version}}/passport) 進行 OAuth 驗證。

當透過 OAuth 驗證您的 MCP 伺服器時，請在您的 `routes/ai.php` 檔案中調用 `Mcp::oauthRoutes` 方法，以註冊所需的 OAuth2 探索與用戶端註冊路由。接著，在 `routes/ai.php` 檔案中的 `Mcp::web` 路由套用 Passport 的 `auth:api` 中介層：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```


#### 新的 Passport 安裝

如果您的應用程式尚未開始使用 Laravel Passport，請按照 Passport 的 [安裝與部署指南](/docs/{{version}}/passport#installation) 將 Passport 新增到您的應用程式中。在繼續之前，您應該擁有一個 `OAuthenticatable` 模型、新的身分驗證 Guard 以及 Passport 金鑰。

接下來，您應該發布 Laravel MCP 提供的 Passport 授權視圖：

```shell
php artisan vendor:publish --tag=mcp-views
```

然後，使用 `Passport::authorizationView` 方法指示 Passport 使用此視圖。通常，應在應用程式 `AppServiceProvider` 的 `boot` 方法中調用此方法：

```php
use Laravel\Passport\Passport;

/**
 * Bootstrap any application services.
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

此視圖將在身分驗證期間顯示給終端使用者，用以拒絕或核准 AI 代理程式的身分驗證嘗試。

![授權畫面範例](data:image/png;base64,)

> [!NOTE]
> 在此情境下，我們僅將 OAuth 作為底層可驗證模型的轉譯層。我們忽略了 OAuth 的許多面向，例如 Scopes。


#### 使用現有的 Passport 安裝

如果您的應用程式已經在使用 Laravel Passport，Laravel MCP 應該可以在現有的 Passport 安裝中無縫運作，但目前不支援自定義 Scopes，因為 OAuth 主要被用作底層可驗證模型的轉譯層。

Laravel MCP 透過上述討論的 `Mcp::oauthRoutes` 方法，新增、宣告並使用單一的 `mcp:use` Scope。


#### Passport 與 Sanctum 的比較

OAuth2.1 是 Model Context Protocol 規範中記載的身分驗證機制，也是 MCP 用戶端支援最廣泛的機制。因此，我們建議在可能的情況下使用 Passport。

如果您的應用程式已經在使用 [Sanctum](/docs/{{version}}/sanctum)，那麼新增 Passport 可能會很繁瑣。在這種情況下，我們建議繼續使用 Sanctum 而不使用 Passport，直到您有明確且必要的需求要使用僅支援 OAuth 的 MCP 用戶端。

<a name="sanctum"></a>
### Sanctum

如果您想使用 [Sanctum](/docs/{{version}}/sanctum) 來保護您的 MCP 伺服器，只需在 `routes/ai.php` 檔案中將 Sanctum 的身份驗證中介層加入到您的伺服器即可。接著，請確保您的 MCP 用戶端提供 `Authorization: Bearer <token>` 標頭以確保驗證成功：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```


<a name="custom-mcp-authentication"></a>
#### 自定義 MCP 身份驗證

如果您的應用程式核發自己的自定義 API Token，您可以透過將任何您想要的中介層指派給 `Mcp::web` 路由來驗證您的 MCP 伺服器。您的自定義中介層可以手動檢查 `Authorization` 標頭，以對傳入的 MCP 請求進行身份驗證。

<a name="authorization"></a>
## 授權

您可以透過 `$request->user()` 方法存取當前已通過驗證的使用者，這讓您可以在 MCP 工具與資源中執行 [授權檢查](/docs/{{version}}/authorization)：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```


<a name="testing-servers"></a>
## 測試伺服器

您可以使用內建的 MCP Inspector 或透過撰寫單元測試來測試您的 MCP 伺服器。


<a name="mcp-inspector"></a>
### MCP Inspector

[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是一個用於測試與除錯 MCP 伺服器的互動式工具。使用它可以連線至您的伺服器、驗證身份驗證，並試用工具、資源與提示。

您可以為任何已註冊的伺服器執行 Inspector：

```shell
# Web server...
php artisan mcp:inspector mcp/weather

# Local server named "weather"...
php artisan mcp:inspector weather
```

此命令會啟動 MCP Inspector 並提供客戶端設定，您可以將其複製到您的 MCP 客戶端中，以確保所有設定皆正確無誤。如果您的 Web 伺服器受身份驗證中介層保護，請確保在連線時包含所需的標頭，例如 `Authorization` bearer 權杖。


<a name="unit-tests"></a>
### 單元測試

您可以為您的 MCP 伺服器、工具、資源與提示撰寫單元測試。

若要開始，請建立一個新的測試案例，並在註冊該原語 (Primitive) 的伺服器上調用所需原語。例如，若要測試 `WeatherServer` 上的工具：

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * Test a tool.
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

同樣地，您可以測試提示與資源：

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

您也可以在調用原語之前串接 `actingAs` 方法，藉此以已通過驗證的使用者身份進行操作：

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

收到回應後，您可以使用各種斷言方法來驗證回應的內容與狀態。

您可以使用 `assertOk` 方法斷言回應是否成功。這會檢查回應中是否不包含任何錯誤：

```php
$response->assertOk();
```

您可以使用 `assertSee` 方法斷言回應中是否包含特定文字：

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

您可以使用 `assertHasErrors` 方法斷言回應中是否包含錯誤：

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

您可以使用 `assertHasNoErrors` 方法斷言回應中是否不包含錯誤：

```php
$response->assertHasNoErrors();
```

您可以使用 `assertName()`、`assertTitle()` 與 `assertDescription()` 方法斷言回應中是否包含特定的中繼資料：

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

您可以使用 `assertSentNotification` 與 `assertNotificationCount` 方法斷言通知是否已發送：

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

最後，如果您希望檢查原始回應內容，可以使用 `dd` 或 `dump` 方法來輸出回應以供除錯：

```php
$response->dd();
$response->dump();
```