...
[Laravel MCP](https://github.com/laravel/mcp) 提供了一個簡單優雅的方式，讓 AI 客戶端可以透過 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) 與您的 Laravel 應用程式互動。它提供了一個表達性強、流暢的介面，用於定義伺服器、工具、資源和提示，以實現由 AI 驅動的應用程式互動。
...
Laravel MCP 與 Laravel 的[驗證功能](/docs/{{version}}/validation)完美整合。您可以在工具的 `handle` 方法中驗證傳入的工具引數：
...
Laravel [服務容器](/docs/{{version}}/container)用於解析所有工具。因此，您可以在工具的建構子中指定工具可能需要的任何依賴項。宣告的依賴項將自動解析並注入到工具實例中：
...
您可以透過 [annotations](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 來增強工具，以向 AI 用戶端提供額外中繼資料。這些註解有助於 AI 模型理解工具的行為和功能。註解透過屬性新增到工具中：
...
對於長時間執行的操作或即時資料串流，工具可以從其 `handle` 方法中回傳一個 [產生器](https://www.php.net/manual/en/language.generators.overview.php)。這使得在最終回應之前，可以向客戶端發送中繼更新：
...
[Prompts](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 讓你的伺服器能夠分享可重複使用的提示模板，供 AI 客戶端用於與語言模型互動。它們提供了一種標準化的方式來建構常見的查詢和互動。
...
預設情況下，Prompt 的名稱和標題是從類別名稱派生而來。例如，`DescribeWeatherPrompt` 的名稱將是 `describe-weather`，標題則是 `Describe Weather Prompt`。你可以透過在 Prompt 上定義 `$name` 和 `$title` 屬性來客製化這些值：
...
Laravel 的 [service container](/docs/{{version}}/container) 用於解析所有 Prompts。因此，你可以在 Prompt 的建構函式中類型提示任何它可能需要的依賴。宣告的依賴將會自動解析並注入到 Prompt 實例中：
...
[Resources](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 讓你的伺服器能夠公開資料和內容，供 AI 客戶端在與語言模型互動時讀取和用作上下文。它們提供了一種方式來分享靜態或動態資訊，例如文件、設定或任何有助於 AI 回應的資料。
...
預設情況下，資源的名稱和標題是從類別名稱派生而來。例如，`WeatherGuidelinesResource` 的名稱將是 `weather-guidelines`，標題將是 `Weather Guidelines Resource`。您可以透過在資源上定義 `$name` 和 `$title` 屬性來客製化這些值：
...
Laravel [服務容器](/docs/{{version}}/container)用於解析所有資源。因此，您可以在資源的建構子中型別提示資源可能需要的任何依賴。宣告的依賴將自動解析並注入到資源實例中：
...
認證您的 MCP 伺服器有兩種方式：簡單的基於權杖的認證，透過 [Laravel Sanctum](/docs/{{version}}/sanctum)，或任何透過 `Authorization` HTTP 標頭傳遞的任意 API 權杖。或者，您可以透過 OAuth 使用 [Laravel Passport](/docs/{{version}}/passport) 進行認證。

<a name="oauth"></a>
### OAuth 2.1

保護您的 Web 型 MCP 伺服器最穩健的方式是透過 OAuth 與 [Laravel Passport](/docs/{{version}}/passport)。
...
如果您的應用程式尚未正在使用 Laravel Passport，請先依照 Passport 的[安裝與部署步驟](/docs/{{version}}/passport#installation)進行。在繼續之前，您應該具備 `OAuthenticatable` 模型、新的認證守衛，以及 Passport 金鑰。
...
然後，指示 Passport 使用 `Passport::authorizationView` 方法來使用此視圖。通常，此方法應在應用程式 `AppServiceProvider` 的 `boot` 方法中呼叫：
...
如果您的應用程式已經在使用 [Sanctum](/docs/{{version}}/sanctum)，那麼新增 Passport 可能會很麻煩。在此情況下，我們建議您在尚未有明確且必要的需求，必須使用僅支援 OAuth 的 MCP 客戶端時，暫時繼續使用 Sanctum 而不新增 Passport。

<a name="sanctum"></a>
### Sanctum

如果您希望使用 [Sanctum](/docs/{{version}}/sanctum) 保護您的 MCP 伺服器，只需將 Sanctum 的認證中介層加入到您伺服器的 `routes/ai.php` 檔案中即可。然後，確保您的 MCP 客戶端提供 `Authorization: Bearer <token>` 標頭以確保成功認證：
...
[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是一個互動式工具，用於測試和偵錯您的 MCP 伺服器。您可以使用它來連接到您的伺服器，驗證認證，並試用工具、資源和提示。
...
您可以使用 `assertName()`、`assertTitle()` 和 `assertDescription()` 方法斷言回應包含特定中繼資料：
...
您可以使用 `assertSentNotification` 和 `assertNotificationCount` 方法斷言已發送通知：
...
最後，如果您希望檢查原始回應內容，您可以使用 `dd` 或 `dump` 方法輸出回應以進行偵錯：
