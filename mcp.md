# Laravel MCP

- [簡介](#introduction)
- [安裝](#installation)
    - [發布路由](#publishing-routes)
- [建立伺服器](#creating-servers)
    - [伺服器註冊](#server-registration)
    - [Web 伺服器](#web-servers)
    - [本機伺服器](#local-servers)
- [工具 (Tools)](#tools)
    - [建立工具](#creating-tools)
    - [工具輸入結構 (Input Schemas)](#tool-input-schemas)
    - [工具輸出結構 (Output Schemas)](#tool-output-schemas)
    - [驗證工具引數](#validating-tool-arguments)
    - [工具依賴注入](#tool-dependency-injection)
    - [工具註解 (Annotations)](#tool-annotations)
    - [條件式工具註冊](#conditional-tool-registration)
    - [工具回應](#tool-responses)
- [提示詞 (Prompts)](#prompts)
    - [建立提示詞](#creating-prompts)
    - [提示詞引數](#prompt-arguments)
    - [驗證提示詞引數](#validating-prompt-arguments)
    - [提示詞依賴注入](#prompt-dependency-injection)
    - [條件式提示詞註冊](#conditional-prompt-registration)
    - [提示詞回應](#prompt-responses)
- [資源 (Resources)](#resources)
    - [建立資源](#creating-resources)
    - [資源模板](#resource-templates)
    - [資源 URI 與 MIME 類型](#resource-uri-and-mime-type)
    - [資源請求](#resource-request)
    - [資源依賴注入](#resource-dependency-injection)
    - [資源註解 (Annotations)](#resource-annotations)
    - [條件式資源註冊](#conditional-resource-registration)
    - [資源回應](#resource-responses)
- [App](#apps)
    - [建立 App 資源](#creating-app-resources)
    - [從工具渲染 App](#rendering-apps-from-tools)
    - [App 工具可見性](#app-tool-visibility)
    - [App 設定](#app-configuration)
    - [使用 Boost 建置 App](#building-apps-with-boost)
- [中繼資料 (Metadata)](#metadata)
- [認證](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [授權](#authorization)
- [測試伺服器](#testing-servers)
    - [MCP 檢查員 (Inspector)](#mcp-inspector)
    - [單元測試](#unit-tests)

<a name="introduction"></a>
## 簡介

[Laravel MCP](https://github.com/laravel/mcp) 提供了一種簡單且優雅的方式，讓 AI 客戶端能透過 [模型上下文協議 (Model Context Protocol)](https://modelcontextprotocol.io/docs/getting-started/intro) 與您的 Laravel 應用程式互動。它提供了一個具表達力且流暢的介面來定義伺服器、工具 (Tools)、資源 (Resources) 以及提示詞 (Prompts)，進而實現應用程式中由 AI 驅動的互動功能。


<a name="installation"></a>
## 安裝

首先，請使用 Composer 套件管理員將 Laravel MCP 安裝到您的專案中：

```shell
composer require laravel/mcp
```


<a name="publishing-routes"></a>
### 發布路由

安裝 Laravel MCP 後，執行 `vendor:publish` Artisan 指令來發布 `routes/ai.php` 檔案，您將在該檔案中定義您的 MCP 伺服器：

```shell
php artisan vendor:publish --tag=ai-routes
```

此指令會在您應用程式的 `routes` 目錄下建立 `routes/ai.php` 檔案，您將使用此檔案來註冊您的 MCP 伺服器。


<a name="creating-servers"></a>
## 建立伺服器

您可以使用 `make:mcp-server` Artisan 指令建立 MCP 伺服器。伺服器作為核心通訊點，負責向 AI 客戶端公開工具、資源和提示詞等 MCP 功能：

```shell
php artisan make:mcp-server WeatherServer
```

此指令將在 `app/Mcp/Servers` 目錄中建立一個新的伺服器類別。產生的伺服器類別繼承自 Laravel MCP 的基礎 `Laravel\Mcp\Server` 類別，並提供用於設定伺服器以及註冊工具、資源與提示詞的屬性與屬性值：

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server\Attributes\Instructions;
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Version;
use Laravel\Mcp\Server;

#[Name('Weather Server')]
#[Version('1.0.0')]
#[Instructions('This server provides weather information and forecasts.')]
class WeatherServer extends Server
{
    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // GetCurrentWeatherTool::class,
    ];

    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // WeatherGuidelinesResource::class,
    ];

    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // DescribeWeatherPrompt::class,
    ];
}
```


<a name="server-registration"></a>
### 伺服器註冊

建立伺服器後，您必須在 `routes/ai.php` 檔案中註冊它，使其可以被存取。Laravel MCP 提供了兩種註冊伺服器的方法：`web` 用於可透過 HTTP 存取的伺服器，而 `local` 用於命令列伺服器。


<a name="web-servers"></a>
### Web 伺服器

Web 伺服器是最常見的伺服器類型，可透過 HTTP POST 請求存取，非常適合遠端 AI 客戶端或網頁整合。請使用 `web` 方法註冊 Web 伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

就像一般路由一樣，您可以套用中介層來保護您的 Web 伺服器：

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```


<a name="local-servers"></a>
### 本機伺服器

本機伺服器以 Artisan 指令的形式執行，非常適合建立像 [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost) 這樣的本地 AI 助理整合。請使用 `local` 方法註冊本機伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

註冊完成後，通常不需要手動執行 `mcp:start` Artisan 指令。相反地，請設定您的 MCP 客戶端 (AI 代理) 來啟動伺服器，或是使用 [MCP 檢查員 (Inspector)](#mcp-inspector)。

<a name="tools"></a>
## 工具 (Tools)

工具讓您的伺服器能夠公開 AI 客戶端可以呼叫的功能。它們允許語言模型執行動作、執行程式碼或與外部系統進行互動：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // Get weather...

        return Response::text('The weather is...');
    }

    /**
     * Get the tool's input schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```

<a name="creating-tools"></a>
### 建立工具

若要建立工具，請執行 `make:mcp-tool` Artisan 指令：

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

建立工具後，請將其註冊在伺服器的 `$tools` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```

<a name="tool-name-title-description"></a>
#### 工具名稱、標題與描述

預設情況下，工具的名稱 (Name) 與標題 (Title) 是根據類別名稱衍生而來的。例如，`CurrentWeatherTool` 的名稱將是 `current-weather`，標題則是 `Current Weather Tool`。您可以使用 `Name` 和 `Title` 屬性 (Attributes) 來自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('get-optimistic-weather')]
#[Title('Get Optimistic Weather Forecast')]
class CurrentWeatherTool extends Tool
{
    // ...
}
```

工具的描述不會自動生成。您應該始終使用 `Description` 屬性提供有意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Fetches the current weather forecast for a specified location.')]
class CurrentWeatherTool extends Tool
{
    //
}
```

> [!NOTE]
> 描述是工具中繼資料的關鍵部分，因為它能幫助 AI 模型理解何時以及如何有效地使用該工具。

<a name="tool-input-schemas"></a>
### 工具輸入結構 (Input Schemas)

工具可以定義輸入結構 (Input Schemas) 來指定它們接受來自 AI 客戶端的哪些引數。使用 Laravel 的 `Illuminate\Contracts\JsonSchema\JsonSchema` 建構器來定義工具的輸入需求：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Get the tool's input schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->string()
                ->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```

<a name="tool-output-schemas"></a>
### 工具輸出結構 (Output Schemas)

工具可以定義[輸出結構 (Output Schemas)](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema) 來指定其回應的結構。這能讓需要解析工具結果的 AI 客戶端進行更好的整合。使用 `outputSchema` 方法來定義工具的輸出結構：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Get the tool's output schema.
     *
     * @return array<string, \Illuminate\JsonSchema\Types\Type>
     */
    public function outputSchema(JsonSchema $schema): array
    {
        return [
            'temperature' => $schema->number()
                ->description('Temperature in Celsius')
                ->required(),

            'conditions' => $schema->string()
                ->description('Weather conditions')
                ->required(),

            'humidity' => $schema->integer()
                ->description('Humidity percentage')
                ->required(),
        ];
    }
}
```

<a name="validating-tool-arguments"></a>
### 驗證工具引數

JSON Schema 定義為工具引數提供了基本結構，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的[驗證功能](/docs/{{version}}/validation)無縫整合。您可以在工具的 `handle` 方法中驗證傳入的工具引數：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // Fetch weather data using the validated arguments...
    }
}
```

當驗證失敗時，AI 客戶端將根據您提供的錯誤訊息採取行動。因此，提供清晰且具引導性的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```

<a name="tool-dependency-injection"></a>
#### 工具依賴注入

Laravel [服務容器](/docs/{{version}}/container)被用來解析所有的工具。因此，您可以在工具的建構子中對所需的任何依賴進行型別提示 (Type-hint)。宣告的依賴將自動被解析並注入到工具實例中：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Create a new tool instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構子注入外，您也可以在工具的 `handle()` 方法中對依賴進行型別提示。當該方法被呼叫時，服務容器將自動解析並注入依賴：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```

<a name="tool-annotations"></a>
### 工具註解 (Annotations)

您可以使用 [註解 (annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 來增強您的工具，為 AI 用戶端提供額外的主計資料。這些註解有助於 AI 模型了解工具的行為與功能。註解是透過屬性 (attributes) 新增到工具中的：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

可用的註解包括：

| 註解 | 型別 | 描述 |
| ------------------ | ------- | -------------------------------------------------------------------------------------------- |
| `#[IsReadOnly]` | boolean | 指示工具不會修改其環境。 |
| `#[IsDestructive]` | boolean | 指示工具可能會執行破壞性更新（僅在非唯讀時有意義）。 |
| `#[IsIdempotent]` | boolean | 指示使用相同引數重複呼叫不會產生額外效果（在非唯讀時）。 |
| `#[IsOpenWorld]` | boolean | 指示工具可能會與外部實體互動。 |

註解值可以使用布林引數顯式設定：

```php
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tools\Annotations\IsDestructive;
use Laravel\Mcp\Server\Tools\Annotations\IsOpenWorld;
use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tool;

#[IsReadOnly(true)]
#[IsDestructive(false)]
#[IsOpenWorld(false)]
#[IsIdempotent(true)]
class CurrentWeatherTool extends Tool
{
    //
}
```


<a name="conditional-tool-registration"></a>
### 條件式工具註冊

您可以在執行時期透過在工具類別中實作 `shouldRegister` 方法來條件式地註冊工具。此方法允許您根據應用程式狀態、設定或請求參數來決定工具是否可用：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Determine if the tool should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當工具的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用工具清單中，且無法被 AI 用戶端呼叫。


<a name="tool-responses"></a>
### 工具回應

工具必須回傳 `Laravel\Mcp\Response` 的執行個體。Response 類別提供了多個方便的方法來建立不同類型的回應：

對於簡單的文字回應，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

要指示工具執行期間發生錯誤，請使用 `error` 方法：

```php
return Response::error('Unable to fetch weather data. Please try again.');
```

要回傳圖片或音訊內容，請使用 `image` 與 `audio` 方法：

```php
return Response::image(file_get_contents(storage_path('weather/radar.png')), 'image/png');

return Response::audio(file_get_contents(storage_path('weather/alert.mp3')), 'audio/mp3');
```

您也可以使用 `fromStorage` 方法直接從 Laravel 檔案系統磁碟載入圖片與音訊內容。MIME 類型將自動從檔案中偵測：

```php
return Response::fromStorage('weather/radar.png');
```

如果需要，您可以指定特定磁碟或覆蓋 MIME 類型：

```php
return Response::fromStorage('weather/radar.png', disk: 's3');

return Response::fromStorage('weather/radar.png', mimeType: 'image/webp');
```


<a name="multiple-content-responses"></a>
#### 多重內容回應

工具可以透過回傳 `Response` 執行個體的陣列來回傳多個內容片段：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text("**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F")
    ];
}
```


<a name="structured-responses"></a>
#### 結構化回應

工具可以使用 `structured` 方法回傳 [結構化內容](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content)。這為 AI 用戶端提供了可解析的資料，同時保持與 JSON 編碼文字表示形式的向下相容性：

```php
return Response::structured([
    'temperature' => 22.5,
    'conditions' => 'Partly cloudy',
    'humidity' => 65,
]);
```

如果您需要在結構化內容旁提供自訂文字，請在回應工廠 (response factory) 上使用 `withStructuredContent` 方法：

```php
return Response::make(
    Response::text('Weather is 22.5°C and sunny')
)->withStructuredContent([
    'temperature' => 22.5,
    'conditions' => 'Sunny',
]);
```


<a name="streaming-responses"></a>
#### 串流回應 (Streaming Responses)

對於長時間執行的操作或即時資料串流，工具可以從其 `handle` 方法回傳一個 [產生器 (generator)](https://www.php.net/manual/en/language.generators.overview.php)。這讓您能夠在最終回應之前向用戶端傳送中間更新：

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

當使用 Web 型伺服器時，串流回應會自動開啟一個 SSE (Server-Sent Events) 串流，將每個產出的訊息作為事件傳送給用戶端。

<a name="prompts"></a>
## 提示詞 (Prompts)

[提示詞 (Prompts)](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 讓您的伺服器能夠共享可重用的提示詞模板，供 AI 客戶端用來與語言模型互動。它們提供了一種標準化的方式來組織常見的查詢與互動。

<a name="creating-prompts"></a>
### 建立提示詞

要建立一個提示詞，請執行 `make:mcp-prompt` Artisan 指令：

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

建立提示詞後，請將其註冊到伺服器的 `$prompts` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```

<a name="prompt-name-title-and-description"></a>
#### 提示詞名稱、標題與描述

預設情況下，提示詞的名稱與標題是衍生自類別名稱。例如，`DescribeWeatherPrompt` 將具有 `describe-weather` 的名稱和 `Describe Weather Prompt` 的標題。您可以使用 `Name` 和 `Title` 屬性 (Attributes) 來自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-assistant')]
#[Title('Weather Assistant Prompt')]
class DescribeWeatherPrompt extends Prompt
{
    // ...
}
```

提示詞描述不會自動產生。您應該始終使用 `Description` 屬性提供具有意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Generates a natural-language explanation of the weather for a given location.')]
class DescribeWeatherPrompt extends Prompt
{
    //
}
```

> [!NOTE]
> 描述是提示詞中繼資料的關鍵部分，因為它能幫助 AI 模型理解何時以及如何最佳化地使用該提示詞。

<a name="prompt-arguments"></a>
### 提示詞引數

提示詞可以定義引數，讓 AI 客戶端使用特定值來自定義提示詞模板。請使用 `arguments` 方法來定義您的提示詞接受哪些引數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Get the prompt's arguments.
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```

<a name="validating-prompt-arguments"></a>
### 驗證提示詞引數

提示詞引數會根據其定義自動進行驗證，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在提示詞的 `handle` 方法中驗證傳入的提示詞引數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // Generate the prompt response using the given tone...
    }
}
```

當驗證失敗時，AI 客戶端將根據您提供的錯誤訊息採取行動。因此，提供清晰且具引導性的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```

<a name="prompt-dependency-injection"></a>
### 提示詞依賴注入

Laravel [服務容器(Service Container)](/docs/{{version}}/container) 用於解析所有的提示詞。因此，您可以在提示詞的建構函式中型別提示 (Type-hint) 提示詞所需的任何依賴項。宣告的依賴項將自動被解析並注入到提示詞實例中：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Create a new prompt instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

除了建構函式注入外，您也可以在提示詞的 `handle` 方法中型別提示依賴項。服務容器會在方法被呼叫時自動解析並注入依賴項：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```

<a name="conditional-prompt-registration"></a>
### 條件式提示詞註冊

您可以透過在提示詞類別中實作 `shouldRegister` 方法，在執行時期條件式地註冊提示詞。此方法允許您根據應用程式狀態、設定或請求參數來決定提示詞是否可用：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * Determine if the prompt should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當提示詞的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用提示詞清單中，且無法被 AI 客戶端呼叫。

<a name="prompt-responses"></a>
### 提示詞回應

提示詞可以回傳單個 `Laravel\Mcp\Response` 或一組可疊代的 `Laravel\Mcp\Response` 實例。這些回應封裝了將發送給 AI 客戶端的內容：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

您可以使用 `asAssistant()` 方法來標示該回應訊息應被視為來自 AI 助理，而一般的訊息則被視為使用者輸入。

<a name="resources"></a>
## 資源 (Resources)

[資源 (Resources)](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 讓您的伺服器能夠公開資料和內容，供 AI 客戶端在與語言模型互動時讀取並作為上下文 (Context) 使用。它們提供了一種共享靜態或動態資訊的方式，例如文件、設定或任何有助於提供 AI 回應參考的資料。

<a name="creating-resources"></a>
## 建立資源

若要建立資源，請執行 `make:mcp-resource` Artisan 指令：

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

建立資源後，請將其註冊到伺服器的 `$resources` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```


<a name="resource-name-title-and-description"></a>
#### 資源名稱、標題與描述

預設情況下，資源的名稱和標題是根據類別名稱推導而來的。例如，`WeatherGuidelinesResource` 的名稱將為 `weather-guidelines`，標題則為 `Weather Guidelines Resource`。您可以使用 `Name` 和 `Title` 屬性自定義這些值：

```php
use Laravel\Mcp\Server\Attributes\Name;
use Laravel\Mcp\Server\Attributes\Title;

#[Name('weather-api-docs')]
#[Title('Weather API Documentation')]
class WeatherGuidelinesResource extends Resource
{
    // ...
}
```

資源描述不會自動生成。您應該始終使用 `Description` 屬性提供具有意義的描述：

```php
use Laravel\Mcp\Server\Attributes\Description;

#[Description('Comprehensive guidelines for using the Weather API.')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```

> [!NOTE]
> 描述是資源中繼資料中至關重要的部分，因為它能幫助 AI 模型理解何時以及如何有效地使用該資源。


<a name="resource-templates"></a>
### 資源模板

[資源模板](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates)能讓您的伺服器開放與包含變數的 URI 模式相匹配的動態資源。您不需要為每個資源定義靜態 URI，而是可以建立一個單一資源，根據模板模式處理多個 URI。


<a name="creating-resource-templates"></a>
#### 建立資源模板

若要建立資源模板，請在您的資源類別實作 `HasUriTemplate` 介面，並定義一個回傳 `UriTemplate` 執行個體的 `uriTemplate` 方法：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

#[Description('Access user files by ID')]
#[MimeType('text/plain')]
class UserFileResource extends Resource implements HasUriTemplate
{
    /**
     * Get the URI template for this resource.
     */
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/files/{fileId}');
    }

    /**
     * Handle the resource request.
     */
    public function handle(Request $request): Response
    {
        $userId = $request->get('userId');
        $fileId = $request->get('fileId');

        // Fetch and return the file content...

        return Response::text($content);
    }
}
```

當資源實作了 `HasUriTemplate` 介面時，它將被註冊為資源模板而非靜態資源。AI 用戶端隨後可以使用與模板模式匹配的 URI 來請求資源，URI 中的變數將被自動提取，並在資源的 `handle` 方法中可供使用。


<a name="uri-template-syntax"></a>
#### URI 模板語法

URI 模板使用大括號包圍的佔位符來定義 URI 中的變數段：

```php
new UriTemplate('file://users/{userId}');
new UriTemplate('file://users/{userId}/files/{fileId}');
new UriTemplate('https://api.example.com/{version}/{resource}/{id}');
```


<a name="accessing-template-variables"></a>
#### 存取模板變數

當 URI 與您的資源模板匹配時，提取出的變數會自動合併到請求中，並可以使用 `get` 方法存取：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Contracts\HasUriTemplate;
use Laravel\Mcp\Server\Resource;
use Laravel\Mcp\Support\UriTemplate;

class UserProfileResource extends Resource implements HasUriTemplate
{
    public function uriTemplate(): UriTemplate
    {
        return new UriTemplate('file://users/{userId}/profile');
    }

    public function handle(Request $request): Response
    {
        // Access the extracted variable
        $userId = $request->get('userId');

        // Access the full URI if needed
        $uri = $request->uri();

        // Fetch user profile...

        return Response::text("Profile for user {$userId}");
    }
}
```

`Request` 物件同時提供了提取的變數和原始被請求的 URI，為您處理資源請求提供了完整的上下文。


<a name="resource-uri-and-mime-type"></a>
### 資源 URI 與 MIME 類型

每個資源都由唯一的 URI 識別，並具有相關聯的 MIME 類型，這有助於 AI 用戶端理解資源的格式。

預設情況下，資源的 URI 是根據資源名稱生成的，因此 `WeatherGuidelinesResource` 的 URI 將為 `weather://resources/weather-guidelines`。預設的 MIME 類型為 `text/plain`。

您可以使用 `Uri` 和 `MimeType` 屬性自定義這些值：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Attributes\Uri;
use Laravel\Mcp\Server\Resource;

#[Uri('weather://resources/guidelines')]
#[MimeType('application/pdf')]
class WeatherGuidelinesResource extends Resource
{
}
```

URI 和 MIME 類型有助於 AI 用戶端決定如何適當地處理和解讀資源內容。


<a name="resource-request"></a>
### 資源請求

與工具 (Tools) 和提示詞 (Prompts) 不同，資源不能定義輸入結構 (Input Schemas) 或引數。然而，您仍然可以在資源的 `handle` 方法中與請求物件進行互動：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```


<a name="resource-dependency-injection"></a>
### 資源依賴注入

Laravel 的[服務容器](/docs/{{version}}/container)被用來解析所有資源。因此，您可以在資源的建構子中對任何需要的依賴進行型別提示。宣告的依賴將會被自動解析並注入到資源執行個體中：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Create a new resource instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構子注入外，您也可以在資源的 `handle` 方法中對依賴進行型別提示。服務容器將在方法被呼叫時自動解析並注入依賴：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="resource-annotations"></a>
### 資源註解 (Annotations)

您可以透過 [註解 (Annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#resourceannotations) 來增強您的資源，以提供額外的中繼資料 (Metadata) 給 AI 客戶端。註解是透過 PHP 屬性 (Attributes) 添加到資源中的：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Enums\Role;
use Laravel\Mcp\Server\Annotations\Audience;
use Laravel\Mcp\Server\Annotations\LastModified;
use Laravel\Mcp\Server\Annotations\Priority;
use Laravel\Mcp\Server\Resource;

#[Audience(Role::User)]
#[LastModified('2025-01-12T15:00:58Z')]
#[Priority(0.9)]
class UserDashboardResource extends Resource
{
    //
}
```

可用的註解包括：

| 註解 | 類型 | 說明 |
| ----------------- | ------------- | --------------------------------------------------------------------------- |
| `#[Audience]`     | Role 或 array | 指定目標受眾 (`Role::User`、`Role::Assistant` 或兩者皆是)。 |
| `#[Priority]`     | float         | 介於 0.0 與 1.0 之間的數值，表示資源的重要性。 |
| `#[LastModified]` | string        | ISO 8601 格式的時間戳記，顯示資源最後更新的時間。 |


<a name="conditional-resource-registration"></a>
### 條件式資源註冊

您可以透過在資源類別中實作 `shouldRegister` 方法，在執行期間條件式地註冊資源。此方法允許您根據應用程式狀態、設定或請求參數來決定資源是否可用：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Determine if the resource should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當資源的 `shouldRegister` 方法回傳 `false` 時，該資源將不會出現在可用資源清單中，且 AI 客戶端無法存取。


<a name="resource-responses"></a>
### 資源回應

資源必須回傳 `Laravel\Mcp\Response` 的實例。Response 類別提供了幾種方便的方法來建立不同類型的回應：

對於簡單的文字內容，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the resource request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```


<a name="resource-blob-responses"></a>
#### Blob 回應

若要回傳 Blob 內容，請使用 `blob` 方法並提供 Blob 內容：

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

回傳 Blob 內容時，MIME 類型將由您資源中設定的 MIME 類型決定：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Attributes\MimeType;
use Laravel\Mcp\Server\Resource;

#[MimeType('image/png')]
class WeatherGuidelinesResource extends Resource
{
    //
}
```


<a name="resource-error-responses"></a>
#### 錯誤回應

若要表示在取得資源過程中發生錯誤，請使用 `error()` 方法：

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="apps"></a>
## App

Laravel MCP 支援 [MCP Apps](https://modelcontextprotocol.io/extensions/apps/overview)，這是 Model Context Protocol 的一個擴充功能，允許工具在受支援的主控端 (Host) 中，透過沙盒化的 iframe 渲染互動式 HTML 應用程式。這讓您可以建立儀表板 (Dashboards)、表單、視覺化圖表，以及其他超越純文字回應的豐富體驗。

一個 MCP App 由兩個協同運作的部分組成：

- 一個 **App 資源 (App resource)**，負責回傳應用程式所需的自包含 (Self-contained) HTML。
- 一個 **工具 (Tool)**，使用 `#[RendersApp]` 屬性與該 App 資源連結。當該工具被呼叫時，主控端會取得並渲染連結的資源。

<a name="creating-app-resources"></a>
### 建立 App 資源

您可以使用 `make:mcp-app-resource` Artisan 指令來建立一個 App 資源：

```shell
php artisan make:mcp-app-resource WeatherDashboardApp
```

此指令會建立兩個檔案：一個位於 `app/Mcp/Resources` 的 PHP 類別，以及一個位於 `resources/views/mcp` 的 Blade 視圖。視圖名稱會自動根據類別名稱推導。例如，`WeatherDashboardApp` 會對應到 `mcp.weather-dashboard-app`：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\AppMeta;
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\AppResource;

#[Description('An interactive weather dashboard.')]
#[AppMeta]
class WeatherDashboardApp extends AppResource
{
    /**
     * Handle the app resource request.
     */
    public function handle(Request $request): Response
    {
        return Response::view('mcp.weather-dashboard-app', [
            'title' => $this->title(),
        ]);
    }
}
```

`AppResource` 繼承自基礎的 `Resource` 類別，並會自動設定 MCP Apps 規範所需的 `ui://` URI 方案以及 `text/html;profile=mcp-app` MIME 類型。與任何其他資源一樣，您必須將其註冊到伺服器的 `$resources` 陣列中。

產生的 Blade 視圖會使用 `<x-mcp::app>` 元件，它會渲染一個完整的 HTML 文件，並隨附已打包且可直接使用的客戶端 MCP SDK：

```blade
<x-mcp::app :title="$title">
    <x-slot:head>
        <script type="module">
        createMcpApp(async (app) => {
            document.getElementById('run-btn').addEventListener('click', async () => {
                const result = await app.callServerTool('get-weather-data', {});
                document.getElementById('output').textContent = result.content[0]?.text ?? '';
            });
        });
        </script>
    </x-slot:head>

    <div id="app">
        <button id="run-btn">Refresh</button>
        <p id="output"></p>
    </div>
</x-mcp::app>
```

`createMcpApp` 全域函式由隨附的 SDK 提供，負責處理 iframe 與伺服器之間的連接、套用主控端佈景主題，並提供如 `callServerTool`、`sendMessage`、`openLink` 及事件回呼等輔助方法。關於完整的客戶端 API，請參閱 [MCP Apps 規範](https://modelcontextprotocol.io/extensions/apps/overview)。

<a name="rendering-apps-from-tools"></a>
### 從工具渲染 App

若要顯示 App 資源，請使用 `#[RendersApp]` 屬性將一個工具與其連結。當該工具被呼叫時，Laravel MCP 會在工具中繼資料中包含該資源的 URI，以便主控端可以在沙盒化的 iframe 中渲染該 App：

```php
<?php

namespace App\Mcp\Tools;

use App\Mcp\Resources\WeatherDashboardApp;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Attributes\RendersApp;
use Laravel\Mcp\Server\Tool;

#[RendersApp(resource: WeatherDashboardApp::class)]
class ShowWeatherDashboard extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        return Response::text('Weather dashboard loaded.');
    }
}
```

每當有任何 `AppResource` 被註冊時，Laravel MCP 就會自動宣告支援 `io.modelcontextprotocol/ui` 能力，因此不需要額外的伺服器設定。

<a name="app-tool-visibility"></a>
### App 工具可見性

每個 `#[RendersApp]` 工具都可以透過 `visibility` 引數限制誰可以呼叫它。這對於公開某些僅限 App 使用的私有工具非常有用，這些工具可供 UI 呼叫以載入或重新整理資料，而不會讓這些工具對 AI 模型可見：

```php
use Laravel\Mcp\Server\Attributes\RendersApp;
use Laravel\Mcp\Server\Ui\Enums\Visibility;

#[RendersApp(resource: WeatherDashboardApp::class, visibility: [Visibility::App])]
class GetWeatherData extends Tool
{
    // ...
}
```

`Visibility` 列舉有兩個選項：`Model` 與 `App`，預設為兩者皆是。對於 UI 直接呼叫的後端動作，請使用 `[Visibility::App]`；若要讓工具對 UI 不可用，則使用 `[Visibility::Model]`。

<a name="app-configuration"></a>
### App 設定

App 資源上的 `#[AppMeta]` 屬性可用來設定 iframe 的內容安全政策 (CSP)、瀏覽器權限，以及應包含在視圖 `<head>` 中的任何函式庫腳本：

```php
use Laravel\Mcp\Server\Attributes\AppMeta;
use Laravel\Mcp\Server\Ui\Enums\Library;
use Laravel\Mcp\Server\Ui\Enums\Permission;

#[AppMeta(
    connectDomains: ['https://api.weather.com'],
    permissions: [Permission::Geolocation],
    libraries: [Library::Tailwind, Library::Alpine],
)]
class WeatherDashboardApp extends AppResource
{
    // ...
}
```

`Library` 列舉包含了常見前端函式庫（如 `Library::Tailwind` 與 `Library::Alpine`）預先設定好的 CDN 腳本，且其 CDN 來源會自動合併到 CSP 中。`Permission` 列舉則涵蓋了瀏覽器權限，如 `Camera`、`Microphone`、`Geolocation` 以及 `ClipboardWrite`。

對於需要計算或動態產生的設定，您可以覆寫資源上的 `appMeta` 方法，並使用來自 `Laravel\Mcp\Server\Ui` 命名空間的 `AppMeta`、`Csp` 及 `Permissions` 流暢構建器 (Builders)。

<a name="building-apps-with-boost"></a>
### 使用 Boost 建置 App

Laravel MCP 包含了一個專門用於建置 MCP Apps 的 [Boost](/docs/{{version}}/boost) 技能參考。如果您已安裝 [Laravel Boost](/docs/{{version}}/boost)，您的 AI 開發代理可以呼叫 `mcp-development` 技能，並要求它為您架構 App 資源、Blade 視圖以及連結的工具。

關於完整的協議參考，包含完整的客戶端 API 與結構細節，請參閱官方 [MCP Apps 文件](https://modelcontextprotocol.io/extensions/apps/overview)。

<a name="metadata"></a>
## 中繼資料 (Metadata)

Laravel MCP 也支援 [MCP 規範](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta)中定義的 `_meta` 欄位，這是一些 MCP 客戶端或整合功能所要求的。中繼資料可以套用於所有的 MCP 元件，包含工具、資源、提示詞及其回應。

您可以使用 `withMeta` 方法將中繼資料附加到個別的回應內容：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    return Response::text('The weather is sunny.')
        ->withMeta(['source' => 'weather-api', 'cached' => true]);
}
```

對於套用於整個回應封包的結果級別中繼資料，請使用 `Response::make` 包裝您的回應，並在回傳的回應工廠實例上呼叫 `withMeta`：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\ResponseFactory;

/**
 * Handle the tool request.
 */
public function handle(Request $request): ResponseFactory
{
    return Response::make(
        Response::text('The weather is sunny.')
    )->withMeta(['request_id' => '12345']);
}
```

若要將中繼資料附加到工具、資源或提示詞本身，請在類別上定義 `$meta` 屬性：

```php
use Laravel\Mcp\Server\Attributes\Description;
use Laravel\Mcp\Server\Tool;

#[Description('Fetches the current weather forecast.')]
class CurrentWeatherTool extends Tool
{
    protected ?array $meta = [
        'version' => '2.0',
        'author' => 'Weather Team',
    ];

    // ...
}
```

<a name="authentication"></a>
## 認證

就像路由一樣，你可以使用中介層來認證 web MCP 伺服器。在你的 MCP 伺服器加入認證功能後，使用者必須先通過認證，才能使用該伺服器的任何功能。

認證存取 MCP 伺服器的方式有兩種：透過 [Laravel Sanctum](/docs/{{version}}/sanctum) 進行簡單且基於令牌的認證，或者是透過 `Authorization` HTTP 標頭傳遞的任何令牌。此外，你也可以使用 [Laravel Passport](/docs/{{version}}/passport) 透過 OAuth 進行認證。

<a name="oauth"></a>
### OAuth 2.1

保護網頁版 MCP 伺服器最強大的方式是透過 [Laravel Passport](/docs/{{version}}/passport) 使用 OAuth。

當透過 OAuth 認證您的 MCP 伺服器時，請在您的 `routes/ai.php` 檔案中呼叫 `Mcp::oauthRoutes` 方法，以註冊必要的 OAuth2 探索與用戶端註冊路由。接著，將 Passport 的 `auth:api` 中介層套用到 `routes/ai.php` 檔案中的 `Mcp::web` 路由：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```


#### 全新 Passport 安裝

如果您的應用程式尚未開始使用 Laravel Passport，請遵循 Passport 的 [安裝與部署指南](/docs/{{version}}/passport#installation) 將 Passport 新增到您的應用程式中。在繼續下一步之前，您應該先擁有一個 `OAuthenticatable` 模型、新的認證守衛 (Guard) 以及 Passport 金鑰。

接下來，您應該發布 Laravel MCP 提供的 Passport 授權視圖：

```shell
php artisan vendor:publish --tag=mcp-views
```

然後，使用 `Passport::authorizationView` 方法指示 Passport 使用此視圖。通常，此方法應在應用程式 `AppServiceProvider` 的 `boot` 方法中呼叫：

```php
use Laravel\Passport\Passport;

/**
 * Bootstrap any application services.
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

此視圖將在認證期間顯示給終端使用者，供其拒絕或核准 AI 代理的認證請求。

![授權畫面範例](data:image/png;base64,)

> [!NOTE]
> 在這種情況下，我們僅將 OAuth 用作底層可認證模型的轉換層。我們忽略了 OAuth 的許多面向，例如範圍 (Scopes)。


#### 使用現有的 Passport 安裝

如果您的應用程式已經在使用 Laravel Passport，Laravel MCP 應該可以在您現有的 Passport 安裝中無縫運作，但目前不支援自訂範圍 (Scopes)，因為 OAuth 主要是作為底層可認證模型的轉換層。

Laravel MCP 透過上述討論的 `Mcp::oauthRoutes` 方法，新增、宣告並使用單一的 `mcp:use` 範圍。


#### Passport vs. Sanctum

OAuth 2.1 是 Model Context Protocol 規範中記錄的認證機制，也是 MCP 用戶端支援最廣泛的機制。因此，我們建議在可行時優先使用 Passport。

如果您的應用程式已經在使用 [Sanctum](/docs/{{version}}/sanctum)，那麼新增 Passport 可能會很繁瑣。在這種情況下，我們建議先使用 Sanctum 而不使用 Passport，直到您有明確且必要的技術需求，必須使用僅支援 OAuth 的 MCP 用戶端為止。

<a name="sanctum"></a>
### Sanctum

如果您想要使用 [Sanctum](/docs/{{version}}/sanctum) 保護您的 MCP 伺服器，只需在您的 `routes/ai.php` 檔案中將 Sanctum 的認證中介層加入到伺服器即可。接著，請確保您的 MCP 用戶端提供 `Authorization: Bearer <token>` 標頭以確保認證成功：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```


<a name="custom-mcp-authentication"></a>
#### 自定義 MCP 認證

如果您的應用程式會核發自定義的 API 令牌，您可以透過為 `Mcp::web` 路由指派任何您想要的中介層來對您的 MCP 伺服器進行認證。您的自定義中介層可以手動檢查 `Authorization` 標頭，藉此對傳入的 MCP 請求進行認證。

<a name="authorization"></a>
## 授權

您可以透過 `$request->user()` 方法存取當前已驗證的使用者，這讓您能夠在 MCP 工具與資源中執行 [授權檢查](/docs/{{version}}/authorization)：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```


<a name="testing-servers"></a>
## 測試伺服器

您可以使用內建的 MCP 檢查員 (Inspector) 或撰寫單元測試來測試您的 MCP 伺服器。


<a name="mcp-inspector"></a>
### MCP 檢查員 (Inspector)

[MCP 檢查員 (Inspector)](https://modelcontextprotocol.io/docs/tools/inspector) 是一個用於測試與偵錯 MCP 伺服器的互動式工具。使用它來連接到您的伺服器、驗證認證，並嘗試工具、資源與提示詞。

您可以針對任何已註冊的伺服器執行檢查員：

```shell
# Web server...
php artisan mcp:inspector mcp/weather

# Local server named "weather"...
php artisan mcp:inspector weather
```

此命令會啟動 MCP 檢查員，並提供您可以複製到 MCP 客戶端的客戶端設定，以確保一切設定正確。如果您的 Web 伺服器受認證中介層保護，請在連接時確保包含必要的標頭，例如 `Authorization` 承載令牌 (Bearer token)。


<a name="unit-tests"></a>
### 單元測試

您可以為您的 MCP 伺服器、工具、資源與提示詞撰寫單元測試。

要開始測試，請建立一個新的測試案例，並在註冊該原始元件 (Primitive) 的伺服器上呼叫它。例如，要測試 `WeatherServer` 上的工具：

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * Test a tool.
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

同樣地，您可以測試提示詞與資源：

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

您也可以在呼叫原始元件之前鏈結 `actingAs` 方法，以模擬已驗證的使用者身分：

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

收到回應後，您可以使用各種斷言方法來驗證回應的內容與狀態。

您可以使用 `assertOk` 方法斷言回應是否成功。這會檢查回應是否不包含任何錯誤：

```php
$response->assertOk();
```

您可以使用 `assertSee` 方法斷言回應是否包含特定文字：

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

您可以使用 `assertHasErrors` 方法斷言回應是否包含錯誤：

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

您可以使用 `assertHasNoErrors` 方法斷言回應是否不包含任何錯誤：

```php
$response->assertHasNoErrors();
```

您可以使用 `assertName()`、`assertTitle()` 與 `assertDescription()` 方法斷言回應是否包含特定的中繼資料：

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

您可以使用 `assertSentNotification` 與 `assertNotificationCount` 方法斷言是否已發送通知：

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

最後，如果您希望查看原始回應內容，可以使用 `dd` 或 `dump` 方法來輸出回應以進行偵錯：

```php
$response->dd();
$response->dump();
```