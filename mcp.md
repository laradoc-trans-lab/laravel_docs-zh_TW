...
- [提示詞](#prompts)
    ...
    - [提示詞參數](#prompt-arguments)
    - [驗證提示詞參數](#validating-prompt-arguments)
...
[Laravel MCP](https://github.com/laravel/mcp) 為 AI 客戶端提供了一種簡單且優雅的方式，透過 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) 與您的 Laravel 應用程式進行互動。它提供了一個具表達力且流暢的介面，用於定義伺服器、工具、資源和提示詞，從而實現與您應用程式的 AI 驅動互動。


<a name="installation"></a>
## 安裝

要開始使用，請透過 Composer 套件管理員將 Laravel MCP 安裝到您的專案中：
...
您可以使用 `make:mcp-server` Artisan 指令建立一個 MCP 伺服器。伺服器作為核心通訊點，向 AI 客戶端公開工具、資源和提示詞等 MCP 功能：

```shell
php artisan make:mcp-server WeatherServer
```

此指令將在 `app/Mcp/Servers` 目錄中建立一個新的伺服器類別。產生的伺服器類別繼承了 Laravel MCP 的基礎 `Laravel\Mcp\Server` 類別，並提供了用於設定伺服器以及註冊工具、資源和提示詞的 Attributes 與屬性：
...
建立伺服器後，您必須在 `routes/ai.php` 檔案中註冊它才能使其可供存取。Laravel MCP 提供了兩種註冊伺服器的方法：用於可透過 HTTP 存取之伺服器的 `web`，以及用於命令列伺服器的 `local`。


<a name="web-servers"></a>
### Web 伺服器

Web 伺服器是最常見的伺服器類型，可透過 HTTP POST 請求存取，這使其成為遠端 AI 客戶端或網頁整合的理想選擇。使用 `web` 方法註冊 Web 伺服器：
...
本地端伺服器以 Artisan 指令的形式執行，非常適合建立像 [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost) 這樣的本地 AI 助手整合。使用 `local` 方法註冊本地端伺服器：
...
註冊後，通常不需要手動執行 `mcp:start` Artisan 指令。相反地，請設定您的 MCP 客戶端 (AI 代理) 來啟動伺服器，或使用 [MCP Inspector](#mcp-inspector)。

<a name="tools"></a>
## 工具

工具讓您的伺服器能夠公開 AI 用戶端可以呼叫的功能。它們允許語言模型執行操作、執行程式碼或與外部系統互動：
...
預設情況下，工具的名稱和標題是根據類別名稱衍生的。例如，`CurrentWeatherTool` 的名稱將為 `current-weather`，標題將為 `Current Weather Tool`。您可以使用 `Name` 和 `Title` 屬性來自定義這些值：
...
工具描述不會自動產生。您應該始終使用 `Description` 屬性提供具有意義的描述：
...
> [!NOTE]
> 描述是工具中繼資料的關鍵部分，因為它有助於 AI 模型理解何時以及如何有效地使用該工具。


<a name="tool-input-schemas"></a>
### 工具輸入 Schema

工具可以定義輸入 Schema，以指定它們接受來自 AI 用戶端的哪些參數。使用 Laravel 的 `Illuminate\Contracts\JsonSchema\JsonSchema` 建構器來定義工具的輸入需求：
...
工具可以定義 [輸出 Schema](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#output-schema) 以指定其回應的結構。這可以讓需要可解析工具結果的 AI 用戶端進行更好的整合。使用 `outputSchema` 方法來定義工具的輸出結構：
...
JSON Schema 定義提供了工具參數的基本結構，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在工具的 `handle` 方法中驗證傳入的工具參數：
...
當驗證失敗時，AI 用戶端將根據您提供的錯誤訊息進行操作。因此，提供清晰且可操作的錯誤訊息至關重要：
...
Laravel [服務容器](/docs/{{version}}/container) 用於解析所有工具。因此，您可以在工具的建構子中對工具可能需要的任何依賴項進行型別提示 (Type-hint)。宣告的依賴項將自動解析並注入到工具實例中：
...
除了建構子注入外，您也可以在工具的 `handle()` 方法中對依賴項進行型別提示。當方法被呼叫時，服務容器將自動解析並注入依賴項：
...
您可以使用 [註解 (Annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 來強化您的工具，以向 AI 用戶端提供額外的中繼資料。這些註解有助於 AI 模型理解工具的行為與能力。註解是透過屬性 (Attributes) 加入至工具中的：
...
| 註解 | 類型 | 描述 |
| ------------------ | ------- | -------------------------------------------------------------------------------------------- |
| `#[IsReadOnly]`    | boolean | 指出工具不會修改其環境。 |
| `#[IsDestructive]` | boolean | 指出工具可能會執行破壞性更新（僅在非唯讀時有意義）。 |
| `#[IsIdempotent]`  | boolean | 指出使用相同參數重複呼叫不會產生額外影響（當非唯讀時）。 |
| `#[IsOpenWorld]`   | boolean | 指出工具可能會與外部實體互動。 |
...
您可以透過在工具類別中實作 `shouldRegister` 方法，在執行期間條件式地註冊工具。此方法允許您根據應用程式狀態、設定或請求參數來決定工具是否可用：
...
當工具的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用工具清單中，且無法被 AI 用戶端調用。


<a name="tool-responses"></a>
### 工具回應

工具必須回傳一個 `Laravel\Mcp\Response` 實例。Response 類別提供了幾個方便的方法來建立不同類型的回應：
...
工具可以透過回傳一個 `Response` 實例陣列來回傳多個內容片段：
...
工具可以使用 `structured` 方法回傳 [結構化內容](https://modelcontextprotocol.io/specification/2025-06-18/server/tools#structured-content)。這為 AI 用戶端提供了可解析的資料，同時保持與 JSON 編碼文字表示形式的向後相容性：
...
如果您需要在結構化內容旁提供自定義文字，請在 Response 工廠上使用 `withStructuredContent` 方法：
...
對於長時間執行的操作或即時資料串流，工具可以從其 `handle` 方法回傳一個 [產生器 (Generator)](https://www.php.net/manual/en/language.generators.overview.php)。這使得在最終回應之前可以向用戶端傳送中間更新：
...
當使用基於 Web 的伺服器時，串流回應會自動開啟一個 SSE (Server-Sent Events) 串流，將每個產出的訊息作為事件傳送給用戶端。

<a name="prompts"></a>
## 提示詞

[提示詞](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 讓您的伺服器能夠共享可重複使用的提示詞模板，AI 客戶端可以使用這些模板與語言模型進行互動。它們提供了一種標準化的方式來建構常見的查詢與互動。
...
預設情況下，提示詞的名稱和標題源自於類別名稱。例如，`DescribeWeatherPrompt` 將具有 `describe-weather` 的名稱和 `Describe Weather Prompt` 的標題。您可以使用 `Name` 和 `Title` 屬性來自定義這些值：
...
提示詞描述不會自動產生。您應該始終使用 `Description` 屬性提供有意義的描述：
...
> [!NOTE]
> 描述是提示詞中繼資料的關鍵部分，因為它能幫助 AI 模型理解何時以及如何最佳地利用該提示詞。


<a name="prompt-arguments"></a>
### 提示詞參數

提示詞可以定義參數，允許 AI 客戶端使用特定值來客製化提示詞模板。請使用 `arguments` 方法來定義您的提示詞接受哪些參數：
...
提示詞參數會根據其定義自動進行驗證，但您可能還想實施更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在提示詞的 `handle` 方法中驗證傳入的提示詞參數：
...
Laravel [服務容器](/docs/{{version}}/container) 用於解析所有提示詞。因此，您可以在提示詞的建構子中對提示詞可能需要的任何依賴項進行型別提示。宣告的依賴項將自動解析並注入到提示詞實例中：
...
除了建構子注入外，您還可以在提示詞的 `handle` 方法中對依賴項進行型別提示。服務容器將在呼叫該方法時自動解析並注入依賴項：
...
您可以透過在提示詞類別中實作 `shouldRegister` 方法，在執行階段條件式地註冊提示詞。此方法允許您根據應用程式狀態、設定或請求參數來決定提示詞是否可用：
...
提示詞可以回傳單個 `Laravel\Mcp\Response` 或 `Laravel\Mcp\Response` 實例的可迭代物件。這些回應封裝了將傳送給 AI 客戶端的內容：
...
您可以使用 `asAssistant()` 方法來表示回應訊息應被視為來自 AI 助手，而一般訊息則被視為使用者輸入。


<a name="resources"></a>
## 資源

[資源](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 讓您的伺服器能夠公開資料與內容，AI 客戶端可以在與語言模型互動時讀取並將其作為上下文使用。它們提供了一種方式來共享靜態或動態資訊，如文件、設定或任何有助於提供 AI 回應資訊的資料。

<a name="creating-resources"></a>
## 建立資源

要建立一個 Resource，請執行 `make:mcp-resource` Artisan 指令：

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

建立 Resource 後，請將其註冊在伺服器的 `$resources` 屬性中：
...
預設情況下，Resource 的名稱與標題是根據類別名稱衍生而來的。例如，`WeatherGuidelinesResource` 的名稱將為 `weather-guidelines`，標題為 `Weather Guidelines Resource`。您可以使用 `Name` 與 `Title` 屬性來自訂這些值：
...
Resource 的描述不會自動產生。您應該始終使用 `Description` 屬性來提供有意義的描述：
...
> [!NOTE]
> 描述是 Resource 中繼資料的重要部分，因為它能幫助 AI 模型理解何時以及如何有效地使用該 Resource。


<a name="resource-templates"></a>
### 資源模板

[資源模板](https://modelcontextprotocol.io/specification/2025-06-18/server/resources#resource-templates) 讓您的伺服器能夠公開與包含變數的 URI 模式相匹配的動態資源。您不需要為每個資源定義靜態 URI，而是可以建立單個 Resource，根據模板模式處理多個 URI。


<a name="creating-resource-templates"></a>
#### 建立資源模板

要建立資源模板，請在您的 Resource 類別實作 `HasUriTemplate` 介面，並定義一個回傳 `UriTemplate` 實例的 `uriTemplate` 方法：
...
當 Resource 實作 `HasUriTemplate` 介面時，它將被註冊為資源模板而非靜態資源。AI 用戶端接著可以使用符合模板模式的 URI 請求資源，且 URI 中的變數將自動被提取，並可在您的 Resource 的 `handle` 方法中使用。


<a name="uri-template-syntax"></a>
#### URI 模板語法

URI 模板使用大括號包圍的佔位符來定義 URI 中的變數片段：
...
當 URI 符合您的資源模板時，提取的變數會自動合併到請求中，並可以使用 `get` 方法進行存取：
...
`Request` 物件同時提供了提取的變數與被請求的原始 URI，為您處理資源請求提供完整的上下文。


<a name="resource-uri-and-mime-type"></a>
### 資源 URI 與 MIME 類型

每個 Resource 都由一個唯一的 URI 識別，並具有相關聯的 MIME 類型，幫助 AI 用戶端理解資源的格式。

預設情況下，Resource 的 URI 是根據 Resource 的名稱產生的，因此 `WeatherGuidelinesResource` 的 URI 將為 `weather://resources/weather-guidelines`。預設的 MIME 類型為 `text/plain`。

您可以使用 `Uri` 與 `MimeType` 屬性來自訂這些值：
...
URI 與 MIME 類型能幫助 AI 用戶端決定如何適當地處理與解讀資源內容。


<a name="resource-request"></a>
### 資源請求

與工具和提示詞不同，Resource 無法定義輸入 Schema 或參數。然而，您仍然可以在 Resource 的 `handle` 方法中與請求物件進行互動：
...
Laravel [服務容器](/docs/{{version}}/container) 用於解析所有 Resource。因此，您可以在 Resource 的建構子中對所需的任何依賴項進行型別提示 (Type-hint)。宣告的依賴項將自動被解析並注入到 Resource 實例中：
...
除了建構子注入外，您也可以在 Resource 的 `handle` 方法中對依賴項進行型別提示。服務容器將在呼叫該方法時自動解析並注入依賴項：
...
您可以透過 [註解 (Annotations)](https://modelcontextprotocol.io/specification/2025-06-18/schema#resourceannotations) 增強您的資源，以向 AI 用戶端提供額外的中繼資料。註解是透過屬性 (Attributes) 加入到資源中的：
...
| 註解 | 類型 | 描述 |
| ----------------- | ------------- | --------------------------------------------------------------------------- |
| `#[Audience]` | Role 或陣列 | 指定預期的目標對象 (`Role::User`、`Role::Assistant` 或兩者皆是)。 |
| `#[Priority]` | float | 一個介於 0.0 到 1.0 之間的數值，表示資源的重要性。 |
| `#[LastModified]` | string | 一個 ISO 8601 格式的時間戳記，顯示資源最後更新的時間。 |


<a name="conditional-resource-registration"></a>
### 條件式資源註冊

您可以透過在資源類別中實作 `shouldRegister` 方法，在執行時期條件式地註冊資源。此方法允許您根據應用程式狀態、設定或請求參數來決定資源是否可用：
...
當資源的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用資源清單中，且無法被 AI 用戶端存取。


<a name="resource-responses"></a>
### 資源回應

資源必須回傳 `Laravel\Mcp\Response` 的實例。Response 類別提供了多個方便的方法來建立不同類型的回應：
...
Laravel MCP 也支援 [MCP 規格](https://modelcontextprotocol.io/specification/2025-06-18/basic#meta) 中定義的 `_meta` 欄位，這對某些 MCP 客戶端或整合功能來說是必要的。中繼資料可以套用到所有 MCP 原語 (primitives)，包含工具、資源、提示詞及其回應。
...
對於套用至整個回應封包 (envelope) 的結果層級中繼資料，請使用 `Response::make` 包裝您的回應，並在回傳的回應工廠實例上呼叫 `withMeta`：
...
就像路由一樣，你可以使用中介層對 Web MCP 伺服器進行身分驗證。為你的 MCP 伺服器加入身分驗證，將要求使用者在執行伺服器的任何功能之前必須先經過身分驗證。

有兩種方式可以驗證對 MCP 伺服器的存取：透過 [Laravel Sanctum](/docs/{{version}}/sanctum) 的簡單權杖型 (Token-based) 身分驗證，或是透過 `Authorization` HTTP 標頭傳遞的任何權杖。或者，你也可以使用 [Laravel Passport](/docs/{{version}}/passport) 透過 OAuth 進行驗證。

<a name="oauth"></a>
### OAuth 2.1

保護您基於 Web 的 MCP 伺服器最穩健的方式是透過 [Laravel Passport](/docs/{{version}}/passport) 使用 OAuth。

當透過 OAuth 驗證您的 MCP 伺服器時，請在 `routes/ai.php` 檔案中呼叫 `Mcp::oauthRoutes` 方法，以註冊必要的 OAuth2 探索與用戶端註冊路由。接著，將 Passport 的 `auth:api` 中介層套用到您 `routes/ai.php` 檔案中的 `Mcp::web` 路由：
...
如果您的應用程式尚未開始使用 Laravel Passport，請按照 Passport 的 [安裝與部署指南](/docs/{{version}}/passport#installation) 將 Passport 新增到您的應用程式中。在繼續之前，您應該具備一個 `OAuthenticatable` 模型、新的驗證守衛 (Guard) 以及 Passport 金鑰。

接著，您應該發布 Laravel MCP 提供的 Passport 授權視圖：
...
此視圖將在身分驗證期間顯示給終端使用者，供其拒絕或核准 AI 代理程式的身分驗證嘗試。
...
> [!NOTE]
> 在此情境下，我們僅將 OAuth 作為底層可驗證模型的轉譯層。我們忽略了 OAuth 的許多面向，例如範圍 (Scope)。


#### 使用現有的 Passport 安裝

如果您的應用程式已經在使用 Laravel Passport，Laravel MCP 應該能無縫運作於現有的 Passport 安裝中，但目前不支援自定義範圍 (Scope)，因為 OAuth 主要被用作底層可驗證模型的轉譯層。
...
OAuth2.1 是 Model Context Protocol 規範中記載的身分驗證機制，也是 MCP 用戶端支援最廣泛的機制。因此，我們建議在可能的情況下使用 Passport。

如果您的應用程式已經在使用 [Sanctum](/docs/{{version}}/sanctum)，那麼新增 Passport 可能會很繁瑣。在這種情況下，我們建議繼續使用 Sanctum 而不搭配 Passport，直到您有明確且必要的需求，必須使用僅支援 OAuth 的 MCP 用戶端為止。

<a name="sanctum"></a>
### Sanctum

如果您想使用 [Sanctum](/docs/{{version}}/sanctum) 來保護您的 MCP 伺服器，只需在 `routes/ai.php` 檔案中將 Sanctum 的身分驗證中介層加入您的伺服器。接著，請確保您的 MCP 客戶端提供 `Authorization: Bearer <token>` 標頭以確保成功驗證：
...
如果您的應用程式核發自己的自定義 API 權杖，您可以透過將任何您想要的中介層指派給您的 `Mcp::web` 路由來驗證您的 MCP 伺服器。您的自定義中介層可以手動檢查 `Authorization` 標頭以驗證傳入的 MCP 請求。

<a name="authorization"></a>
## 授權

您可以透過 `$request->user()` 方法存取目前已通過身分驗證的使用者，這讓您可以在 MCP 工具與資源中進行 [授權檢查](/docs/{{version}}/authorization)：
...
[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是用於測試與除錯 MCP 伺服器的互動式工具。使用它來連接到您的伺服器、驗證身分驗證，並嘗試工具、資源與提示詞。

您可以為任何已註冊的伺服器執行檢測器 (Inspector)：
...
此命令會啟動 MCP Inspector 並提供客戶端設定，您可以將這些設定複製到您的 MCP 客戶端中，以確保一切配置正確。如果您的 Web 伺服器受到身分驗證中介層的保護，請確保在連接時包含所需的標頭，例如 `Authorization` bearer token。
...
首先，建立一個新的測試案例，並在註冊該基本元件的伺服器上呼叫它。例如，要在 `WeatherServer` 上測試一個工具：
...
您也可以在呼叫基本元件之前鏈結 `actingAs` 方法，以已驗證使用者的身分進行操作：
...
您可以使用 `assertOk` 方法來斷言回應是成功的。這會檢查回應是否包含任何錯誤：

```php
$response->assertOk();
```

您可以使用 `assertSee` 方法來斷言回應包含特定的文字：

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

您可以使用 `assertHasErrors` 方法來斷言回應包含錯誤：
...
您可以使用 `assertHasNoErrors` 方法來斷言回應不包含錯誤：

```php
$response->assertHasNoErrors();
```

您可以使用 `assertName()`、`assertTitle()` 與 `assertDescription()` 方法來斷言回應包含特定的中繼資料：
...
您可以使用 `assertSentNotification` 與 `assertNotificationCount` 方法來斷言通知已發送：
...
最後，如果您希望檢查原始回應內容，可以使用 `dd` 或 `dump` 方法來輸出回應以進行除錯：
