# Laravel MCP

- [介紹](#introduction)
- [安裝](#installation)
    - [發佈路由](#publishing-routes)
- [建立伺服器](#creating-servers)
    - [伺服器註冊](#server-registration)
    - [Web 伺服器](#web-servers)
    - [本地伺服器](#local-servers)
- [工具](#tools)
    - [建立工具](#creating-tools)
    - [工具輸入 Schema](#tool-input-schemas)
    - [驗證工具引數](#validating-tool-arguments)
    - [工具依賴注入](#tool-dependency-injection)
    - [工具註解](#tool-annotations)
    - [條件式工具註冊](#conditional-tool-registration)
    - [工具回應](#tool-responses)
- [提示](#prompts)
    - [建立提示](#creating-prompts)
    - [提示引數](#prompt-arguments)
    - [驗證提示引數](#validating-prompt-arguments)
    - [提示依賴注入](#prompt-dependency-injection)
    - [條件式提示註冊](#conditional-prompt-registration)
    - [提示回應](#prompt-responses)
- [資源](#resources)
    - [建立資源](#creating-resources)
    - [資源 URI 與 MIME 類型](#resource-uri-and-mime-type)
    - [資源請求](#resource-request)
    - [資源依賴注入](#resource-dependency-injection)
    - [條件式資源註冊](#conditional-resource-registration)
    - [資源回應](#resource-responses)
- [認證](#authentication)
    - [OAuth 2.1](#oauth)
    - [Sanctum](#sanctum)
- [授權](#authorization)
- [測試伺服器](#testing-servers)
    - [MCP Inspector](#mcp-inspector)
    - [單元測試](#unit-tests)

<a name="introduction"></a>
## 介紹

[Laravel MCP](https://github.com/laravel/mcp) 提供了一種簡單優雅的方式，讓 AI 客戶端透過 [Model Context Protocol](https://modelcontextprotocol.io/docs/getting-started/intro) 與您的 Laravel 應用程式進行互動。它提供了一個富有表達性且流暢的介面，用於定義伺服器、工具、資源和提示，以實現 AI 驅動的應用程式互動。


<a name="installation"></a>
## 安裝

要開始使用，請使用 Composer 套件管理器將 Laravel MCP 安裝到您的專案中：

```shell
composer require laravel/mcp
```


<a name="publishing-routes"></a>
### 發佈路由

安裝 Laravel MCP 後，執行 `vendor:publish` Artisan 指令以發佈 `routes/ai.php` 檔案，您將在此定義 MCP 伺服器：

```shell
php artisan vendor:publish --tag=ai-routes
```

此指令將會建立您應用程式 `routes` 目錄中的 `routes/ai.php` 檔案，您將用來註冊 MCP 伺服器。


<a name="creating-servers"></a>
## 建立伺服器

您可以使用 `make:mcp-server` Artisan 指令建立 MCP 伺服器。伺服器作為中央通訊點，向 AI 客戶端公開 MCP 功能，例如工具、資源和提示：

```shell
php artisan make:mcp-server WeatherServer
```

此指令將會在 `app/Mcp/Servers` 目錄中建立一個新的伺服器類別。生成的伺服器類別繼承了 Laravel MCP 的 `Laravel\Mcp\Server` 基礎類別，並提供了用於註冊工具、資源和提示的屬性：

```php
<?php

namespace App\Mcp\Servers;

use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The MCP server's name.
     */
    protected string $name = 'Weather Server';

    /**
     * The MCP server's version.
     */
    protected string $version = '1.0.0';

    /**
     * The MCP server's instructions for the LLM.
     */
    protected string $instructions = 'This server provides weather information and forecasts.';

    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        // GetCurrentWeatherTool::class,
    ];

    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        // WeatherGuidelinesResource::class,
    ];

    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        // DescribeWeatherPrompt::class,
    ];
}
```


<a name="server-registration"></a>
### 伺服器註冊

建立伺服器後，您必須在 `routes/ai.php` 檔案中註冊它，使其可存取。Laravel MCP 提供了兩種註冊伺服器的方法：`web` 用於 HTTP 可存取的伺服器，以及 `local` 用於命令列伺服器。


<a name="web-servers"></a>
### Web 伺服器

Web 伺服器是最常見的伺服器類型，可透過 HTTP POST 請求存取，使其成為遠端 AI 客戶端或基於 Web 整合的理想選擇。使用 `web` 方法註冊一個 web 伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/weather', WeatherServer::class);
```

就像一般的路由一樣，您可以應用中介層來保護您的 web 伺服器：

```php
Mcp::web('/mcp/weather', WeatherServer::class)
    ->middleware(['throttle:mcp']);
```


<a name="local-servers"></a>
### 本地伺服器

本地伺服器以 Artisan 指令的形式執行，非常適合建立本地 AI 助理整合，例如 [Laravel Boost](/docs/{{version}}/installation#installing-laravel-boost)。使用 `local` 方法註冊一個本地伺服器：

```php
use App\Mcp\Servers\WeatherServer;
use Laravel\Mcp\Facades\Mcp;

Mcp::local('weather', WeatherServer::class);
```

註冊後，您通常不需要手動執行 `mcp:start` Artisan 指令。相反地，請配置您的 MCP 客戶端 (AI 代理) 以啟動伺服器，或使用 [MCP Inspector](#mcp-inspector)。

<a name="tools"></a>
## 工具

工具讓您的伺服器能夠向 AI 客戶端公開功能。它們允許語言模型執行動作、執行程式碼或與外部系統互動：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * The tool's description.
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $location = $request->get('location');

        // Get weather...

        return Response::text('The weather is...');
    }

    /**
     * Get the tool's input schema.
     *
     * @return array<string, \Illuminate\JsonSchema\JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),
        ];
    }
}
```


<a name="creating-tools"></a>
### 建立工具

要建立一個工具，請執行 `make:mcp-tool` Artisan 指令：

```shell
php artisan make:mcp-tool CurrentWeatherTool
```

建立工具後，請將其註冊到您伺服器的 `$tools` 屬性中：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Tools\CurrentWeatherTool;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The tools registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Tool>>
     */
    protected array $tools = [
        CurrentWeatherTool::class,
    ];
}
```


<a name="tool-name-title-description"></a>
#### 工具名稱、標題與描述

預設情況下，工具的名稱與標題是從類別名稱衍生而來。例如，`CurrentWeatherTool` 的名稱將是 `current-weather`，標題則是 `Current Weather Tool`。您可以透過定義工具的 `$name` 與 `$title` 屬性來客製化這些值：

```php
class CurrentWeatherTool extends Tool
{
    /**
     * The tool's name.
     */
    protected string $name = 'get-optimistic-weather';

    /**
     * The tool's title.
     */
    protected string $title = 'Get Optimistic Weather Forecast';

    // ...
}
```

工具的描述不會自動生成。您應該始終透過在工具上定義 `$description` 屬性來提供有意義的描述：

```php
class CurrentWeatherTool extends Tool
{
    /**
     * The tool's description.
     */
    protected string $description = 'Fetches the current weather forecast for a specified location.';

    //
}
```

> [!NOTE]
> 描述是工具中繼資料的關鍵部分，因為它有助於 AI 模型理解何時以及如何有效地使用該工具。


<a name="tool-input-schemas"></a>
### 工具輸入 Schema

工具可以定義輸入 Schema，以指定它們從 AI 客戶端接受哪些引數。請使用 Laravel 的 `Illuminate\JsonSchema\JsonSchema` 建構器來定義工具的輸入需求：

```php
<?php

namespace App\Mcp\Tools;

use Illuminate\JsonSchema\JsonSchema;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Get the tool's input schema.
     *
     * @return array<string, JsonSchema>
     */
    public function schema(JsonSchema $schema): array
    {
        return [
            'location' => $schema->string()
                ->description('The location to get the weather for.')
                ->required(),

            'units' => $schema->array()
                ->enum(['celsius', 'fahrenheit'])
                ->description('The temperature units to use.')
                ->default('celsius'),
        ];
    }
}
```


<a name="validating-tool-arguments"></a>
### 驗證工具引數

JSON Schema 定義為工具引數提供了基本的結構，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在工具的 `handle` 方法中驗證傳入的工具引數：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'location' => 'required|string|max:100',
            'units' => 'in:celsius,fahrenheit',
        ]);

        // Fetch weather data using the validated arguments...
    }
}
```

驗證失敗時，AI 客戶端將根據您提供的錯誤訊息採取行動。因此，提供清晰且可操作的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'location' => ['required','string','max:100'],
    'units' => 'in:celsius,fahrenheit',
],[
    'location.required' => 'You must specify a location to get the weather for. For example, "New York City" or "Tokyo".',
    'units.in' => 'You must specify either "celsius" or "fahrenheit" for the units.',
]);
```


<a name="tool-dependency-injection"></a>
#### 工具依賴注入

Laravel [服務容器](/docs/{{version}}/container) 用於解析所有工具。因此，您可以在工具的建構函式中型別提示工具可能需要的任何依賴。聲明的依賴將會自動解析並注入到工具實例中：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Create a new tool instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構函式注入之外，您還可以在工具的 `handle()` 方法中型別提示依賴。當方法被呼叫時，服務容器會自動解析並注入這些依賴：

```php
<?php

namespace App\Mcp\Tools;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $location = $request->get('location');

        $forecast = $weather->getForecastFor($location);

        // ...
    }
}
```


<a name="tool-annotations"></a>
### 工具註解

您可以使用 [註解](https://modelcontextprotocol.io/specification/2025-06-18/schema#toolannotations) 來增強您的工具，為 AI 客戶端提供額外的中繼資料。這些註解有助於 AI 模型理解工具的行為和能力。註解透過屬性添加到工具中：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Server\Tools\Annotations\IsIdempotent;
use Laravel\Mcp\Server\Tools\Annotations\IsReadOnly;
use Laravel\Mcp\Server\Tool;

#[IsIdempotent]
#[IsReadOnly]
class CurrentWeatherTool extends Tool
{
    //
}
```

可用的註解包括：

| 註解               | 類型     | 描述                                                                |
| ------------------ | -------- | ------------------------------------------------------------------- |
| `#[IsReadOnly]`    | 布林值   | 表示工具不會修改其環境。                                            |
| `#[IsDestructive]` | 布林值   | 表示工具可能會執行破壞性更新 (僅在非唯讀時有意義)。                 |
| `#[IsIdempotent]`  | 布林值   | 表示使用相同引數重複呼叫不會產生額外效果 (僅在非唯讀時有意義)。     |
| `#[IsOpenWorld]`   | 布林值   | 表示工具可能會與外部實體互動。                                      |

<a name="conditional-tool-registration"></a>
### 條件式工具註冊

您可以透過在 tool 類別中實作 `shouldRegister` 方法，在執行時條件式地註冊工具。此方法允許您根據應用程式狀態、配置或請求參數來判斷工具是否可用：

```php
<?php

namespace App\Mcp\Tools;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Determine if the tool should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當 tool 的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用工具列表中，且無法被 AI clients 呼叫。

<a name="tool-responses"></a>
### 工具回應

工具必須回傳一個 `Laravel\Mcp\Response` 實例。Response 類別提供了數種便捷方法來建立不同類型的回應：

對於簡單的文字回應，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text('Weather Summary: Sunny, 72°F');
}
```

若要表示工具執行期間發生錯誤，請使用 `error` 方法：

```php
return Response::error('Unable to fetch weather data. Please try again.');
```

<a name="multiple-content-responses"></a>
#### 多重內容回應

工具可以透過回傳 `Response` 實例的陣列來回傳多個內容片段：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 *
 * @return array<int, \Laravel\Mcp\Response>
 */
public function handle(Request $request): array
{
    // ...

    return [
        Response::text('Weather Summary: Sunny, 72°F'),
        Response::text('**Detailed Forecast**\n- Morning: 65°F\n- Afternoon: 78°F\n- Evening: 70°F')
    ];
}
```

<a name="streaming-responses"></a>
#### 串流回應

對於長時間執行的操作或即時資料串流，工具可以從其 `handle` 方法回傳一個 [generator](https://www.php.net/manual/en/language.generators.overview.php)。這使得在最終回應之前可以向客戶端發送中間更新：

```php
<?php

namespace App\Mcp\Tools;

use Generator;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Tool;

class CurrentWeatherTool extends Tool
{
    /**
     * Handle the tool request.
     *
     * @return \Generator<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): Generator
    {
        $locations = $request->array('locations');

        foreach ($locations as $index => $location) {
            yield Response::notification('processing/progress', [
                'current' => $index + 1,
                'total' => count($locations),
                'location' => $location,
            ]);

            yield Response::text($this->forecastFor($location));
        }
    }
}
```

當使用基於 Web 的伺服器時，串流回應會自動開啟一個 SSE (Server-Sent Events, 伺服器傳送事件) 串流，將每個產生的訊息作為事件發送給客戶端。

<a name="prompts"></a>
## 提示

[Prompts](https://modelcontextprotocol.io/specification/2025-06-18/server/prompts) 讓您的伺服器能夠共用可重複使用的 Prompt 範本，AI 用戶端可藉此與語言模型互動。它們提供了一種標準化的方式來結構化常見的查詢和互動。


<a name="creating-prompts"></a>
### 建立提示

要建立一個 Prompt，請執行 `make:mcp-prompt` Artisan 指令：

```shell
php artisan make:mcp-prompt DescribeWeatherPrompt
```

建立 Prompt 後，請在您的伺服器的 `$prompts` 屬性中註冊它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Prompts\DescribeWeatherPrompt;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The prompts registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Prompt>>
     */
    protected array $prompts = [
        DescribeWeatherPrompt::class,
    ];
}
```


<a name="prompt-name-title-and-description"></a>
#### 提示名稱、標題與說明

預設情況下，Prompt 的名稱和標題是來自類別名稱。例如，`DescribeWeatherPrompt` 將具有 `describe-weather` 的名稱和 `Describe Weather Prompt` 的標題。您可以透過在 Prompt 上定義 `$name` 和 `$title` 屬性來自訂這些值：

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * The prompt's name.
     */
    protected string $name = 'weather-assistant';

    /**
     * The prompt's title.
     */
    protected string $title = 'Weather Assistant Prompt';

    // ...
}
```

Prompt 說明不會自動產生。您應始終透過在您的 Prompts 上定義 `$description` 屬性來提供有意義的說明：

```php
class DescribeWeatherPrompt extends Prompt
{
    /**
     * The prompt's description.
     */
    protected string $description = 'Generates a natural-language explanation of the weather for a given location.';

    //
}
```

> [!NOTE]
> 說明是 Prompt 後設資料的關鍵部分，因為它有助於 AI 模型了解何時以及如何最佳地利用 Prompt。


<a name="prompt-arguments"></a>
### 提示引數

Prompts 可以定義引數，允許 AI 用戶端使用特定值自訂 Prompt 範本。使用 `arguments` 方法來定義您的 Prompt 接受哪些引數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Server\Prompt;
use Laravel\Mcp\Server\Prompts\Argument;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Get the prompt's arguments.
     *
     * @return array<int, \Laravel\Mcp\Server\Prompts\Argument>
     */
    public function arguments(): array
    {
        return [
            new Argument(
                name: 'tone',
                description: 'The tone to use in the weather description (e.g., formal, casual, humorous).',
                required: true,
            ),
        ];
    }
}
```


<a name="validating-prompt-arguments"></a>
### 驗證提示引數

Prompt 引數會根據其定義自動驗證，但您可能還想強制執行更複雜的驗證規則。

Laravel MCP 與 Laravel 的 [驗證功能](/docs/{{version}}/validation) 無縫整合。您可以在您的 Prompt 的 `handle` 方法中驗證傳入的 Prompt 引數：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            'tone' => 'required|string|max:50',
        ]);

        $tone = $validated['tone'];

        // Generate the prompt response using the given tone...
    }
}
```

驗證失敗時，AI 用戶端將根據您提供的錯誤訊息行事。因此，提供清晰且可執行的錯誤訊息至關重要：

```php
$validated = $request->validate([
    'tone' => ['required','string','max:50'],
],[
    'tone.*' => 'You must specify a tone for the weather description. Examples include "formal", "casual", or "humorous".',
]);
```


<a name="prompt-dependency-injection"></a>
### 提示依賴注入

Laravel [服務容器](/docs/{{version}}/container) 用於解析所有 Prompts。因此，您可以在 Prompt 的建構函式中類型提示任何 Prompt 可能需要的依賴。宣告的依賴將自動被解析並注入到 Prompt 實例中：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Create a new prompt instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    //
}
```

除了建構函式注入之外，您還可以在 Prompt 的 `handle` 方法中類型提示依賴。當方法被呼叫時，服務容器將自動解析並注入依賴：

```php
<?php

namespace App\Mcp\Prompts;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     */
    public function handle(Request $request, WeatherRepository $weather): Response
    {
        $isAvailable = $weather->isServiceAvailable();

        // ...
    }
}
```


<a name="conditional-prompt-registration"></a>
### 條件式提示註冊

您可以透過在 Prompt 類別中實作 `shouldRegister` 方法，在執行時條件式地註冊 Prompts。此方法允許您根據應用程式狀態、設定或請求參數來判斷一個 Prompt 是否應該可用：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Prompt;

class CurrentWeatherPrompt extends Prompt
{
    /**
     * Determine if the prompt should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當一個 Prompt 的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用 Prompts 的列表中，也無法被 AI 用戶端呼叫。


<a name="prompt-responses"></a>
### 提示回應

Prompts 可以回傳單個 `Laravel\Mcp\Response` 實例或 `Laravel\Mcp\Response` 實例的疊代器。這些回應封裝了將會傳送給 AI 用戶端的內容：

```php
<?php

namespace App\Mcp\Prompts;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Prompt;

class DescribeWeatherPrompt extends Prompt
{
    /**
     * Handle the prompt request.
     *
     * @return array<int, \Laravel\Mcp\Response>
     */
    public function handle(Request $request): array
    {
        $tone = $request->string('tone');

        $systemMessage = "You are a helpful weather assistant. Please provide a weather description in a {$tone} tone.";

        $userMessage = "What is the current weather like in New York City?";

        return [
            Response::text($systemMessage)->asAssistant(),
            Response::text($userMessage),
        ];
    }
}
```

您可以使用 `asAssistant()` 方法來表示回應訊息應被視為來自 AI 助理，而常規訊息則被視為使用者輸入。


<a name="resources"></a>
## 資源

[Resources](https://modelcontextprotocol.io/specification/2025-06-18/server/resources) 讓您的伺服器能夠公開資料和內容，AI 用戶端可以讀取並用作與語言模型互動時的上下文。它們提供了一種分享靜態或動態資訊的方式，例如文件、設定或任何有助於提供 AI 回應的資料。

<a name="creating-resources"></a>
## 建立資源

若要建立資源，請執行 `make:mcp-resource` Artisan 命令：

```shell
php artisan make:mcp-resource WeatherGuidelinesResource
```

建立資源後，請在伺服器的 `$resources` 屬性中註冊它：

```php
<?php

namespace App\Mcp\Servers;

use App\Mcp\Resources\WeatherGuidelinesResource;
use Laravel\Mcp\Server;

class WeatherServer extends Server
{
    /**
     * The resources registered with this MCP server.
     *
     * @var array<int, class-string<\Laravel\Mcp\Server\Resource>>
     */
    protected array $resources = [
        WeatherGuidelinesResource::class,
    ];
}
```

<a name="resource-name-title-and-description"></a>
#### 資源名稱、標題與描述

預設情況下，資源的名稱和標題是從類別名稱衍生而來的。例如，`WeatherGuidelinesResource` 將具有 `weather-guidelines` 的名稱和 `Weather Guidelines Resource` 的標題。您可以透過在資源上定義 `$name` 和 `$title` 屬性來客製化這些值：

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * The resource's name.
     */
    protected string $name = 'weather-api-docs';

    /**
     * The resource's title.
     */
    protected string $title = 'Weather API Documentation';

    // ...
}
```

資源描述不會自動產生。您應該始終透過在資源上定義 `$description` 屬性來提供有意義的描述：

```php
class WeatherGuidelinesResource extends Resource
{
    /**
     * The resource's description.
     */
    protected string $description = 'Comprehensive guidelines for using the Weather API.';

    //
}
```

> [!NOTE]
> 描述是資源中繼資料的關鍵部分，因為它有助於 AI 模型理解何時以及如何有效使用資源。

<a name="resource-uri-and-mime-type"></a>
### 資源 URI 與 MIME 類型

每個資源都由一個唯一的 URI 識別，並具有相關聯的 MIME 類型，可協助 AI 用戶端理解資源的格式。

預設情況下，資源的 URI 是根據資源的名稱產生，因此 `WeatherGuidelinesResource` 將具有 `weather://resources/weather-guidelines` 的 URI。預設的 MIME 類型為 `text/plain`。

您可以透過在資源上定義 `$uri` 和 `$mimeType` 屬性來客製化這些值：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * The resource's URI.
     */
    protected string $uri = 'weather://resources/guidelines';

    /**
     * The resource's MIME type.
     */
    protected string $mimeType = 'application/pdf';
}
```

URI 和 MIME 類型可協助 AI 用戶端判斷如何適當地處理和解釋資源內容。

<a name="resource-request"></a>
### 資源請求

與工具和提示不同，資源無法定義輸入 schema 或引數。然而，您仍然可以在資源的 `handle` 方法中與請求物件互動：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(Request $request): Response
    {
        // ...
    }
}
```

<a name="resource-dependency-injection"></a>
### 資源依賴注入

Laravel 的 [服務容器](/docs/{{version}}/container) 用於解析所有資源。因此，您可以在資源的建構子中型別提示任何資源可能需要的依賴。宣告的依賴將自動解析並注入到資源實例中：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Create a new resource instance.
     */
    public function __construct(
        protected WeatherRepository $weather,
    ) {}

    // ...
}
```

除了建構子注入，您也可以在資源的 `handle` 方法中型別提示依賴。當方法被呼叫時，服務容器將自動解析並注入依賴：

```php
<?php

namespace App\Mcp\Resources;

use App\Repositories\WeatherRepository;
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Handle the resource request.
     */
    public function handle(WeatherRepository $weather): Response
    {
        $guidelines = $weather->guidelines();

        return Response::text($guidelines);
    }
}
```

<a name="conditional-resource-registration"></a>
### 條件式資源註冊

您可以透過在資源類別中實作 `shouldRegister` 方法來在執行時條件式註冊資源。此方法允許您根據應用程式狀態、設定或請求參數來判斷資源是否應該可用：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Request;
use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * Determine if the resource should be registered.
     */
    public function shouldRegister(Request $request): bool
    {
        return $request?->user()?->subscribed() ?? false;
    }
}
```

當資源的 `shouldRegister` 方法回傳 `false` 時，它將不會出現在可用資源列表中，AI 用戶端也無法存取。

<a name="resource-responses"></a>
### 資源回應

資源必須回傳 `Laravel\Mcp\Response` 的實例。Response 類別提供了幾種便捷的方法來建立不同類型的回應：

對於簡單的文字內容，請使用 `text` 方法：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the resource request.
 */
public function handle(Request $request): Response
{
    // ...

    return Response::text($weatherData);
}
```

<a name="resource-blob-responses"></a>
#### Blob 回應

若要回傳 blob 內容，請使用 `blob` 方法，提供 blob 內容：

```php
return Response::blob(file_get_contents(storage_path('weather/radar.png')));
```

回傳 blob 內容時，MIME 類型將由資源類別上 `$mimeType` 屬性的值決定：

```php
<?php

namespace App\Mcp\Resources;

use Laravel\Mcp\Server\Resource;

class WeatherGuidelinesResource extends Resource
{
    /**
     * The resource's MIME type.
     */
    protected string $mimeType = 'image/png';

    //
}
```

<a name="resource-error-responses"></a>
#### 錯誤回應

若要指示資源擷取期間發生錯誤，請使用 `error()` 方法：

```php
return Response::error('Unable to fetch weather data for the specified location.');
```

<a name="authentication"></a>
## 認證

您可以像驗證路由一樣，使用中介層來認證 Web MCP 伺服器。這將要求使用者在啟用伺服器的任何功能之前進行認證。

有兩種方式可以認證對您 MCP 伺服器的存取：透過 [Laravel Sanctum](/docs/{{version}}/sanctum) 進行簡單的基於 token 的認證，或透過 `Authorization` HTTP header 傳遞任何其他的 API tokens。或者，您可以使用 [Laravel Passport](/docs/{{version}}/passport) 透過 OAuth 進行認證。

<a name="oauth"></a>
### OAuth 2.1

保護基於 Web 的 MCP 伺服器最穩健的方法是透過 [Laravel Passport](/docs/{{version}}/passport) 使用 OAuth。

當透過 OAuth 認證您的 MCP 伺服器時，您將在 `routes/ai.php` 檔案中呼叫 `Mcp::oauthRoutes` 方法來註冊所需的 OAuth2 探索與客戶端註冊路由。然後，將 Passport 的 `auth:api` 中介層套用至您在 `routes/ai.php` 檔案中的 `Mcp::web` 路由：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::oauthRoutes();

Mcp::web('/mcp/weather', WeatherExample::class)
    ->middleware('auth:api');
```

#### 新 Passport 安裝

如果您的應用程式尚未搭配 Laravel Passport 使用，請先遵循 Passport 的 [安裝與部署步驟](/docs/{{version}}/passport#installation)。在繼續之前，您應該會有所屬的 `OAuthenticatable` 模型、新的認證守衛以及 Passport 密鑰。

接著，您應該發佈 Laravel MCP 提供的 Passport 授權視圖：

```shell
php artisan vendor:publish --tag=mcp-views
```

然後，使用 `Passport::authorizationView` 方法指示 Passport 使用此視圖。通常，此方法應在您的應用程式 `AppServiceProvider` 的 `boot` 方法中呼叫：

```php
use Laravel\Passport\Passport;

/**
 * Bootstrap any application services.
 */
public function boot(): void
{
    Passport::authorizationView(function ($parameters) {
        return view('mcp.authorize', $parameters);
    });
}
```

在認證期間，此視圖將會顯示給終端使用者，以拒絕或批准 AI 代理程式的認證嘗試。

![授權畫面範例](data:image/png;base64,)

> [!NOTE]
> 在此情境下，我們僅將 OAuth 用作基礎 Authenticatable model 的轉換層。我們忽略了 OAuth 的許多方面，例如 scopes。

#### 使用現有的 Passport 安裝

如果您的應用程式已經在使用 Laravel Passport，Laravel MCP 應該可以在您現有的 Passport 安裝中無縫地運作，但自訂 scopes 目前不支援，因為 OAuth 主要用作基礎 Authenticatable model 的轉換層。

Laravel MCP 透過上述討論的 `Mcp::oauthRoutes()` 方法，新增、公開並使用單一的 `mcp:use` scope。

#### Passport 與 Sanctum 的比較

OAuth2.1 是 Model Context Protocol 規範中文件化的認證機制，也是 MCP 客戶端中最廣泛支援的。因此，我們建議盡可能地使用 Passport。

如果您的應用程式已經在使用 [Sanctum](/docs/{{version}}/sanctum)，那麼新增 Passport 可能會很繁瑣。在此情況下，我們建議使用 Sanctum 而非 Passport，直到您有明確且必要的需求，需要使用僅支援 OAuth 的 MCP 客戶端。

<a name="sanctum"></a>
### Sanctum

如果您想使用 [Sanctum](/docs/{{version}}/sanctum) 保護您的 MCP 伺服器，只需在 `routes/ai.php` 檔案中將 Sanctum 的認證中介層新增到您的伺服器。接著，請確保您的 MCP 用戶端提供 `Authorization: Bearer <token>` 標頭以確保認證成功：

```php
use App\Mcp\Servers\WeatherExample;
use Laravel\Mcp\Facades\Mcp;

Mcp::web('/mcp/demo', WeatherExample::class)
    ->middleware('auth:sanctum');
```

<a name="custom-mcp-authentication"></a>
#### 自訂 MCP 認證

如果您的應用程式發行自己的自訂 API token，您可以透過將任何您希望的中介層指派給 `Mcp::web` 路由來認證您的 MCP 伺服器。您的自訂中介層可以手動檢查 `Authorization` 標頭以認證傳入的 MCP 請求。

<a name="authorization"></a>
## 授權

您可以透過 `$request->user()` 方法存取目前已認證的使用者，讓您可以在您的 MCP 工具與資源中執行 [授權檢查](/docs/{{version}}/authorization)：

```php
use Laravel\Mcp\Request;
use Laravel\Mcp\Response;

/**
 * Handle the tool request.
 */
public function handle(Request $request): Response
{
    if (! $request->user()->can('read-weather')) {
        return Response::error('Permission denied.');
    }

    // ...
}
```


<a name="testing-servers"></a>
## 測試伺服器

您可以使用內建的 MCP Inspector 或透過撰寫單元測試來測試您的 MCP 伺服器。


<a name="mcp-inspector"></a>
### MCP Inspector

[MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) 是一個互動式工具，用於測試和除錯您的 MCP 伺服器。您可以使用它來連線到您的伺服器、驗證認證，並試用工具、資源與提示。

您可以為任何已註冊的伺服器執行 Inspector：

```shell
```

# Web 伺服器...
php artisan mcp:inspector mcp/weather

# 名為 "weather" 的本地伺服器...
php artisan mcp:inspector weather
```

此指令會啟動 MCP Inspector 並提供用戶端設定，您可以將這些設定複製到您的 MCP 用戶端以確保一切配置正確。如果您的 web 伺服器受到認證中介層保護，請務必在連線時包含所需的標頭，例如 `Authorization` Bearer 權杖。

<a name="unit-tests"></a>
### 單元測試

您可以為您的 MCP 伺服器、工具、資源和提示撰寫單元測試。

要開始使用，請建立一個新的測試案例，並在註冊原始物件的伺服器上呼叫所需的原始物件。例如，若要在 `WeatherServer` 上測試工具：

```php tab=Pest
test('tool', function () {
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
});
```

```php tab=PHPUnit
/**
 * Test a tool.
 */
public function test_tool(): void
{
    $response = WeatherServer::tool(CurrentWeatherTool::class, [
        'location' => 'New York City',
        'units' => 'fahrenheit',
    ]);

    $response
        ->assertOk()
        ->assertSee('The current weather in New York City is 72°F and sunny.');
}
```

同樣地，您可以測試提示和資源：

```php
$response = WeatherServer::prompt(...);
$response = WeatherServer::resource(...);
```

您也可以透過在呼叫原始物件前串聯 `actingAs` 方法，以通過認證的使用者身份進行操作：

```php
$response = WeatherServer::actingAs($user)->tool(...);
```

一旦您收到回應，就可以使用各種斷言方法來驗證回應的內容和狀態。

您可以使用 `assertOk` 方法來斷言回應是否成功。這會檢查回應是否沒有任何錯誤：

```php
$response->assertOk();
```

您可以使用 `assertSee` 方法來斷言回應包含特定文字：

```php
$response->assertSee('The current weather in New York City is 72°F and sunny.');
```

您可以使用 `assertHasErrors` 方法來斷言回應包含錯誤：

```php
$response->assertHasErrors();

$response->assertHasErrors([
    'Something went wrong.',
]);
```

您可以使用 `assertHasNoErrors` 方法來斷言回應不包含錯誤：

```php
$response->assertHasNoErrors();
```

您可以使用 `assertName()`、`assertTitle()` 和 `assertDescription()` 方法來斷言回應包含特定的詮釋資料：

```php
$response->assertName('current-weather');
$response->assertTitle('Current Weather Tool');
$response->assertDescription('Fetches the current weather forecast for a specified location.');
```

您可以使用 `assertSentNotification` 和 `assertNotificationCount` 方法來斷言通知是否已發送：

```php
$response->assertSentNotification('processing/progress', [
    'step' => 1,
    'total' => 5,
]);

$response->assertSentNotification('processing/progress', [
    'step' => 2,
    'total' => 5,
]);

$response->assertNotificationCount(5);
```

最後，如果您希望檢查原始回應內容，可以使用 `dd` 或 `dump` 方法輸出回應以進行偵錯：

```php
$response->dd();
$response->dump();
```