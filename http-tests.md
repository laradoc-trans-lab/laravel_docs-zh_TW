...
Laravel 的 Session 通常用於維護目前已通過身份驗證使用者的狀態。因此，`actingAs` 輔助方法提供了一種簡單的方法來將指定使用者驗證為目前使用者。例如，我們可以使用 [模型工廠 (model factory)](/docs/{{version}}/eloquent-factories) 來產生並驗證使用者：
...
您可以在發送請求之前調用 `withoutExceptionHandling` 方法，完全禁用給定請求的例外處理：

```php
$response = $this->withoutExceptionHandling()->get('/');
```

此外，如果您想確保應用程式沒有使用 PHP 語言或應用程式所使用的函式庫中已棄用 (Deprecated) 的功能，您可以在發送請求之前調用 `withoutDeprecationHandling` 方法。當禁用棄用處理時，棄用警告將會轉換為例外，從而導致您的測試失敗：

```php
$response = $this->withoutDeprecationHandling()->get('/');
```

`assertThrows` 方法可用於斷言給定閉包 (Closure) 中的程式碼拋出了指定類型的例外：
...
如前所述，`assertJson` 方法可用於斷言 JSON 回應中是否存在某個 JSON 片段。如果您想驗證指定的陣列是否**完全符合**應用程式傳回的 JSON，您應該使用 `assertExactJson` 方法：
...
如果您想驗證 JSON 回應在指定路徑上是否包含給定的資料，您應該使用 `assertJsonPath` 方法：
...
如果需要，您可以向該方法傳遞一個 `$mimeType` 參數，以明確定義檔案應返回的 MIME 類型：
...
某些視圖可能依賴於 [Laravel 提供的全域錯誤包 (Global Error Bag)](/docs/{{version}}/validation#quick-displaying-the-validation-errors) 中共享的錯誤。要使用錯誤訊息填充錯誤包，您可以使用 `withViewErrors` 方法：
...
如有必要，您可以使用 `blade` 方法來評估並渲染原始的 [Blade](/docs/{{version}}/blade) 字串。與 `view` 方法一樣，`blade` 方法會回傳一個 `Illuminate\Testing\TestView` 的實例：
...
您可以使用 `component` 方法來評估並渲染 [Blade 元件](/docs/{{version}}/blade#components)。`component` 方法會回傳一個 `Illuminate\Testing\TestComponent` 的實例：
...
此方法是 [assertJsonStructure](#assert-json-structure) 的更嚴格版本。與 `assertJsonStructure` 不同，如果回應中包含任何未明確包含在預期 JSON 結構中的鍵 (Key)，此方法將會失敗。
...
`assertJson` 方法會將回應轉換為陣列，以驗證指定的陣列是否存在於應用程式回傳的 JSON 回應中。因此，即使 JSON 回應中還有其他屬性，只要指定的片段存在，此測試仍會通過。
...
> [!NOTE]
> 更通用的 [assertValid](#assert-valid) 方法可用於斷言回應沒有以 JSON 形式回傳的驗證錯誤，**且**沒有錯誤被閃存 (Flash) 到 Session 儲存空間中。
...
> [!NOTE]
> 更通用的 [assertInvalid](#assert-invalid) 方法可用於斷言回應具有以 JSON 形式回傳的驗證錯誤，**或**有錯誤被閃存到 Session 儲存空間中。
...
斷言回應是否重新導向回上一個頁面，且 [Session 包含指定的錯誤](#assert-session-has-errors)：
...
斷言回應是重新導向至指定的 [簽名路由](/docs/{{version}}/urls#signed-urls)：
...
如果需要，可以將一個閉包作為 `assertSessionHas` 方法的第二個參數。如果閉包回傳 `true`，則斷言通過：
...
斷言 Session 在 [閃存的輸入陣列](/docs/{{version}}/responses#redirecting-with-flashed-session-data) 中具有指定的值：

```php
$response->assertSessionHasInput($key, $value = null);
```

如果需要，可以將一個閉包作為 `assertSessionHasInput` 方法的第二個參數。如果閉包回傳 `true`，則斷言通過：
...
例如，如果您的應用程式 Session 包含 `name` 和 `status` 鍵，您可以像這樣斷言兩者都存在且具有指定的值：
...
斷言 Session 在特定的 [錯誤包 (Error Bag)](/docs/{{version}}/validation#named-error-bags) 中包含指定 `$keys` 的錯誤。如果 `$keys` 是一個關聯陣列，則斷言 Session 在該錯誤包中，為每個欄位 (鍵) 包含特定的錯誤訊息 (值)：
...
> [!NOTE]
> 更通用的 [assertValid](#assert-valid) 方法可用於斷言回應沒有以 JSON 形式回傳的驗證錯誤，**且**沒有錯誤被閃存到 Session 儲存空間中。
...
您也可以斷言指定鍵具有特定的驗證錯誤訊息。執行此操作時，您可以提供完整的訊息或僅提供訊息的一小部分：
...
將閉包作為 `assertViewHas` 方法的第二個參數，可讓您檢查指定的視圖資料片段並對其進行斷言：
...
您也可以斷言給定的鍵值具有特定的驗證錯誤訊息。執行此操作時，您可以提供完整訊息或僅提供訊息的一小部分：
