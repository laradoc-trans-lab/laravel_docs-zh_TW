...
Laravel 提供了多個輔助函式，協助您為應用程式產生 URL。這些輔助函式主要用於在您的樣板和 API 回應中建立連結，或在產生重新導向回應到應用程式的其他部分時。
...
`url` 輔助函式可用於為您的應用程式產生任意 URL。產生的 URL 將自動使用應用程式目前處理的請求中的協定 (HTTP 或 HTTPS) 和主機：
...
提供已存在於路徑中的查詢字串參數將會覆寫其現有值：

    echo url()->query('/posts?sort=latest', ['sort' => 'oldest']);

    // http://example.com/posts?sort=oldest

陣列值也可以作為查詢參數傳遞。這些值將在產生的 URL 中正確地被鍵入和編碼：
...
這些方法都可以透過 `URL` [Facade](/docs/{{version}}/facades) 存取：
...
`route` 輔助函式可用於產生指向[具名路由](/docs/{{version}}/routing#named-routes)的 URL。具名路由讓您可以在不與路由上定義的實際 URL 耦合的情況下產生 URL。因此，如果路由的 URL 發生變化，您的 `route` 函式呼叫無需進行任何更改。例如，假設您的應用程式包含一個定義如下的路由：
...
當然，`route` 輔助函式也可以用於產生帶有多個參數的路由 URL：
...
任何不對應於路由定義參數的額外陣列元素都將被新增到 URL 的查詢字串中：
...
例如，您可以使用簽章 URL 來實作一個公開的「取消訂閱」連結，並透過電子郵件發送給您的客戶。若要為具名路由建立簽章 URL，請使用 `URL` Facade 的 `signedRoute` 方法：

    use Illuminate\Support\Facades\URL;

    return URL::signedRoute('unsubscribe', ['user' => 1]);

您可以透過向 `signedRoute` 方法提供 `absolute` 參數來從簽章 URL 雜湊中排除網域：

    return URL::signedRoute('unsubscribe', ['user' => 1], absolute: false);

如果您想產生一個在指定時間後過期的臨時簽章路由 URL，您可以使用 `temporarySignedRoute` 方法。當 Laravel 驗證臨時簽章路由 URL 時，它將確保編碼到簽章 URL 中的過期時間戳尚未過期：
...
有時，您可能需要允許應用程式的前端向簽章 URL 附加資料，例如在執行客戶端分頁時。因此，您可以使用 `hasValidSignatureWhileIgnoring` 方法指定在驗證簽章 URL 時應忽略的請求查詢參數。請記住，忽略參數允許任何人修改請求上的這些參數：
...
如果您的簽章 URL 不包含 URL 雜湊中的網域，您應該向 Middleware 提供 `relative` 參數：
...
如果 Controller 方法接受路由參數，您可以將路由參數的關聯陣列作為第二個參數傳遞給函式：
...
設定 URL 預設值可能會干擾 Laravel 對隱式 Model 綁定的處理。因此，您應該[優先處理您的 Middleware](/docs/{{version}}/middleware#sorting-middleware)，使其在 Laravel 自己的 `SubstituteBindings` Middleware 之前執行。您可以使用應用程式 `bootstrap/app.php` 檔案中的 `priority` Middleware 方法來實現此目的：
