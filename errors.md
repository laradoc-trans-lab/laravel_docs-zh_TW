...
在內部，Laravel 已經為您忽略了一些類型的錯誤，例如由 404 HTTP 錯誤或無效 CSRF Token 產生的 419 HTTP 回應所導致的例外。如果您想指示 Laravel 停止忽略給定類型的例外，您可以使用應用程式 `bootstrap/app.php` 檔案中的 `stopIgnoring` 例外方法：
